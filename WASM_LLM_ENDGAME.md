...
* **Throughput: ✅ GATE CLEARED — 5.9 tok/s (was 0.6; gate >2.0), Phase 5.5, 2026-06-19.** Root cause was `fused_attention.wgsl` `@workgroup_size(1)` — one thread/head doing the entire Q/K/V projection GEMM serially (~15 threads on a 5000-core Ampere). **Fix (Option B, commit `73489421`):** route Q/K/V projection through the parallel `fused_transformer.wgsl` GEMM (proj buffers + `proj_row_stride`), attention kernel SDPA-only. forward 52437→4452 ms (~12×); GPU drain 1577→114 ms/tok (~14×); coherent `Paris.`. The four prior hypotheses (dispatch fusion `261ecb32`, resident logits `f05ec0c6`, resident norms `d66b30d9`, single-submit `493b0900`) each gave ~0 tok/s — real correctness/hygiene wins, but none touched the attention shader. argmax (~20 ms/tok) is now the largest single item. See `wasm_llm_planning.md` log ar–az.
