...
assert.equal(WASM_ASSET_VERSION, '0.0.30-wasm-retry1');
...
assert.equal(
    versioned.href,
    'https://example.test/playground/qualia_core_db_bg.wasm?v=0.0.30-wasm-retry1',
);
...
try {
    ...
    assert.match(calls[0], /\?v=0\.0\.30-wasm-retry1$/);
} finally {
