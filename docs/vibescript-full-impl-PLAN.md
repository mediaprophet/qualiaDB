...
| T42 | **Ship desktop HID loop first** — `hid:pointer:*`, `hid:keyboard:*`, `hid:touch:*`. The hook grammar (`on hid:...`) already parses; nothing dispatches yet. | **Done** — InboundEvent ABI defines pointer/keyboard/touch events. Desktop event loop dispatch wired to PoetSnapshot (commit `af8b0e4a`). `hid.poll()` tested, empty polling returns Null, dispatch and quota behavior covered. | recommendations §4.5 |
...
| T53 | **Wire GBNF into in-process sampling loop** — projectional mutations can wait; logit mask is the actual MCP replacement. | **Done** — DOMINO masker wired into native decode loop with grammar state advancement (feed_token after each selection). VibeScript bindings: `sampler.configure`, `sampler.constrain_enable/disable/reset`, `sampler.sample`. 23 tests (commit `ee48c6f3`). | recommendations §4.3, W11, excellence-first §3.13 |
...
| # | Task | Status | Source |
|---|------|--------|--------|
| T60 | **LSP server** — `tower-lsp` based; autocomplete, go-to-definition, find-references, real-time diagnostics. | **Done** — vibe-lsp crate (renamed from poet-lsp) with JSON-RPC LSP | ecosystem §3.1, topics-yet-considered §1 |
| T61 | **WASM playground** — Monaco editor + live output; zero-install. | **Done** — `crates/vibe-wasm` (wasm-bindgen wrapper, 560KB binary), `docs/vibe/playground.html` (Monaco editor, live eval, bytecode disassembly, 11 examples), `docs/vibe/index.html` (landing page), `docs/vibe/spec.html` (rendered standard with TOC), `docs/vibe/dev-docs.html` (API reference from source — 1040 items across 65 modules), `docs/vibe/showcase.html` (12 auto-running demos), `docs/vibe/benchmarks.html` (VibeScript WASM vs JS, 6 benchmarks with charts). `gen-dev-docs` binary extracts public API from source. | ecosystem §3.2, topics-yet-considered §2 |
...
| T71 | **One clock story in Wellfair / pulse / poet** — `asserted_time_unix: u32` in wellfare-core is another coarse Unix. Replace together, or Vibe Instant won't compose. | **Done** — `InstantBridge` + `DurationBridge` bridge types with full comparison/arithmetic API. `RecordEnvelope` carries `Option<InstantBridge>` fields alongside `u32` (backward-compatible serde). All 14 wellfare-core domain modules now have `InstantBridge` fields + accessors: assessment, authority_attestation, clinical, conditions, credentials, finance, guardianship, life_records, medication, mental_wellbeing, personal_records, projects, welfare_support, record. 177 `InstantBridge` references across wellfare-core. 261 tests pass. | excellence-first §3.14 |
...
| W11 | GBNF on the in-process sampler | **Done** (tracked as T53) — DOMINO masker wired into decode loop with grammar advancement, VibeScript bindings exposed (commit `ee48c6f3`) | excellence-first §4 |
