...
- **Sentinel / Sentinel VM**: Core 1 logic engine. Executes SlgOpcodes (CheckDefeaters, CheckThreshold, etc.) on SlgArena (42MB tabling buffer).
- **SlgOpcode**: Bytecode for the Sentinel (CheckTable, Unify, Halt, BranchWorld, etc.).
- **Modalities**: Specialized reasoning (spatio_temporal, probabilistic, dl, asp, linear, diffusion) normalized to Sentinel.
...
- **did:git**: Git-based decentralized identity for Webizen agency and axiomatic evolution (validated by Sentinel).
- **Webizen**: Protocol for identity nyms/facets, address book, and Commons enforcement.
- **Neurosymbolic Intercept**: Map LLM tensors to Quins so Sentinel can override violations of local Spatio-Temporal contexts.
