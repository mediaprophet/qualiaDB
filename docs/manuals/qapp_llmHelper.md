...
- **Available Tools:** `query_graph`, `get_graph_stats`, `list_ontologies`, `llm_infer`, `llm_chat`, `list_models`, `qpu_optimize`, `qpu_dft`, `qpu_status`, `matrix_operation`, `algebra_solve_polynomial`, `algebra_matrix_analyze`, `cas`, `ode_solve`, `chemical_analysis`, `statistical_analysis`, `ml_inference`, `financial_model`, `medical_score`, `engineering_analysis_op`, `get_wallet_status`, `get_did_info`, `ingest_ontology`, `validate_shacl`, `inject_test_quin`, `list_qapps`, `get_qapp_manifest`, `inspect_qapp_readiness`, `list_qapp_updates`, `describe_qapp_surface_schema`, `get_system_status`, `evaluate_modality`, `bioinformatics_align`, `chemical_descriptors`, `clinical_risk`, `symbolic_logic_infer`, `geometric_algebra_op`, `validate_hardware_assembly`.
  - `algebra_solve_polynomial` — roots (real + complex) of a polynomial from descending coefficients.
  - `algebra_matrix_analyze` — `op ∈ {determinant, eigenvalues, eigen_symmetric, svd}` over a row-major matrix.
  - `cas` — symbolic algebra: `op ∈ {differentiate, simplify, expand, evaluate, solve_quadratic, factor}` over text expressions (e.g. `"x^3 - 2*x^2 + 5"`). See [Q42 Symbolic Algebra Encoding](standards/q42-symbolic-algebra-encoding.md).
  - These three (unlike most demo tools) operate on **caller-supplied** data, not fixed inputs.
...
- **Numeric Algebra:** Matrix multiply/add/transpose/inverse/solve, determinant & LU, symmetric (Jacobi) and general (Faddeev–LeVerrier) eigenvalues, SVD, and polynomial root-finding (quadratic closed form + Durand–Kerner) — via `matrix_operation`, `algebra_matrix_analyze`, `algebra_solve_polynomial`. A privacy-preserving variant proves `A·B = C` in zero knowledge (Groth16, fixed-point).
- **Symbolic Algebra (CAS):** Parse, differentiate, simplify, expand, factor, and symbolically solve expressions; lossless `Expr ↔ NQuin` encoding for storing & citing derived results — via the `cas` tool. Distinct from "Symbolic Rule Inferencing" below (that is SAT / defeasible logic, not computer algebra).
