...
| D6 | Should DisclosureDenied be a `Value` variant (post-0.1 grammar) or a `Result` error kind (0.1 compatible)? | Result error kind first (0.1 compatible); Value variant is post-0.1 | **Resolved by Timothy 2026-08-20** — keep as `Value::DisclosureDenied`. Rights enforcement is correct system execution, not an operational fault. `Value::is_truthy()` returns false, enabling graceful script degradation. DisclosureDenied can be posted to blackboards, passed across DAG nodes, inspected by JudgeFrame, and sealed into VCs — a transient `Result::Err` would be lost on unwind. |
