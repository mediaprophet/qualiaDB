...
| **P2** | GatherDequant + Reduce + Broadcast | those nodes + `WgslLowerer` methods (Reduce lifts the shared-mem tree-reduce; GatherDequant reuses the certified ternary/dequant-into path) + CPU floors; migrate `ternary-gemv` to a `{GatherDequant→MatMul}` template | per-node CPU-vs-GPU certify green on A2000; ternary template certifies == legacy |
| **P3** | Softmax sugar + Elementwise LLM kit + fusion rewrite | full Elementwise fn-set; Softmax legalization; a `MatMul→Elementwise` fusion graph-rewrite reproducing **fused-ffn as emergent** (not a hand id); migrate `fused-ffn` onto it | fused-ffn template certifies == legacy `emit_ffn_wgsl`; softmax graded vs composed CPU floor |
