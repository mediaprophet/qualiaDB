...
| **P4b** | Full attention block + GatherDequant + measured uplift — ✅ DONE | `graph_ops/gather_dequant.rs` (NEW): native ternary `GatherDequant` op-node (2-bit unpack `{0,+1,−1}`×scale, packed words bound as `array<u32>` to dodge f32 NaN-canonicalization) — WGSL + exact CPU oracle + executor arm (GPU + CPU). `executor.rs` builders: `attention_graph` (decode-step `softmax(q·Kᵀ)·V`), `decode_block_graph` (RMSNorm→attn→residual→RMSNorm→SwiGLU-FFN→residual), `dequant_matmul_graph` (`{GatherDequant→MatMul}`). | **GPU-certified on A2000**: attention, `{GatherDequant→MatMul}`, and the full decode block each match their composed CPU oracle; `gather_dequant_gpu_matches_oracle` exact. **Honest uplift (measured, not flattering):** one decode block (d=576,kv=128,ffn=1536, 26 nodes) = GPU **0.07×** the CPU oracle — the Option-A per-node-submit executor (+ per-call context recreation) is **latency-bound**, not faster. This buys correctness+generality; **speed needs the deferred-submit fusion + context-reuse perf pass** (not claimed). Multi-row/prefill attention needs an axis-aware reduce (later). |
...
| **P6** | q42 substrate binding — ✅ DONE | `ir/q42_bridge.rs` (NEW): `serialize_graph`/`deserialize_graph` encode a `ComputeGraph` as 48-byte `NQuin`s — `q42:opKind` (opcode **0x10–0x1A**, reserved-modality range, no `mini_parser`/deontic overlap) + `q42:tensorShape`/`q42:dtype`/`q42:scheduleHint` companions + `q42:feedsInto` edge quins + `q42:graphOutput`; full `u32` matmul dims in the op payload, `u16`/axis shapes (oversize = hard error, no silent truncation); `graph_merkle_root` (blake3 over the NQuin bytes) = provenance. | **Round-trip identity green**: softmax + full **decode-block** graphs round-trip **node-for-node** (`GraphNode: PartialEq`); all 11 op-classes encode/decode; **zero-copy byte path** (serialize→bytes→quins→graph) reproduces the graph + a stable Merkle root; opcode-range + no-collision asserted. (Full `DagStore` VC-ancestry / on-disk volume write is the deeper persistence layer; the in-arena graph stays the source of truth per §5.) |
| **P7** | RT `Neighbor` + remaining backends + Stencil/Scatter — ✅ DONE (RT-core *accel* = optional remainder) | **part 1** `graph_ops/{stencil,scatter}.rs`: native `Stencil` (Laplacian/RopePair; Divergence/Advection need a velocity field+model → `Err`) + `ScatterAccum` (Add/Max via `atomicCompareExchangeWeak` CAS on the u32 bits — no `atomic<f32>` in WGSL). **part 2** `emit/{graph_msl,graph_hlsl}.rs`: `MslLowerer`/`HlslLowerer` for the portable kit (Elementwise/Reduce/Broadcast). **part 3** `graph_ops/neighbor.rs`: exact-grid `Neighbor` (FRNN/kNN/Range) + `legalize` (RT gated to ≤3-D / faithful projection; dims>3 → grid) + `recall_vs_grid` (the §6 differential-oracle measure). | **GPU-certified on A2000**: `stencil_gpu_matches_oracle`, `scatter_gpu_matches_oracle` (4096→64 contention CAS). MSL/HLSL emit + structurally validate (HLSL DXC-validatable with the toolchain; none on this Win/NVIDIA host). `Neighbor` exact-grid + `legalize` + `recall` **hand-checked green** — the correctness floor + the mandatory oracle. **Optional remainder:** the RT-core *acceleration* of the 3-D case (`build_aabb_scene` AABB BLAS + `ray_query` collection, recall-graded vs this grid) — a separate hardware slice; needs wgpu-29 procedural-AABB `ray_query` feasibility verification. The Neighbor op is complete + exact on the grid regardless; RT only speeds the 3-D path. |
