...
### 2026-07-07 - P13.1 implemented (Mesh quality metrics + size/anisotropy fields)
1. STEP + STATUS       - P13.1 (Mesh quality metrics and size/anisotropy fields) - implemented.
2. WHAT WAS BUILT       - New module mesh_quality.rs (~1455 lines). (a) Triangle quality: min/max interior angle, edge ratio, aspect ratio (circumradius/inradius), radius-edge ratio, area, validity; 	ri_signed_area_2d for planar inversion detection. (b) Tet quality: min/max dihedral (6 edges via outward face normals), radius-edge ratio, scaled Jacobian (det/|a||b||c|, range [-1,1], regular tet = sqrt(2)/2), signed/unsigned volume, edge ratio, sliver detection. Circumradius via correct Cramer column-replacement 3x3 solve. (c) Aggregate stats for tri/tet meshes. (d) SizeField (Constant + Background barycentric interpolation w/ extrapolate) and AnisotropyField (Uniform + Background metric-tensor blend) with positive-definiteness check (Sylvester's criterion). (e) Field conformance checking (metric edge length vs unit target, tolerance band). Inverted/degenerate cells fail closed (valid: false). Registered mesh_quality OpManifest (Scalar/Wasm, BitExact, ApproximateMetric, ColdBounded, D3, Implemented).
3. MEASURED RESULTS    - cargo test --lib mesh_quality:: -> 28 passed, 0 failed. Full CG suite: 1378 passed, 0 failed (up from 1350 = +28 net new). capability_manifests tests green.
4. WHERE I NEED THE HUMAN - none blocking.
5. NEXT STEP           - P13.2 (Delaunay refinement for PSLGs with Steiner points), then P13.3, P13.4, P13.5, then P14.1-P14.6, P13.6-P13.10, P14.7-P14.11.


### 2026-07-07 - P13.2 implemented (Delaunay refinement for PSLGs with Steiner points)
1. STEP + STATUS       - P13.2 (Ruppert Delaunay refinement) - implemented.
2. WHAT WAS BUILT       - New module delaunay_refine.rs (~795 lines). Ruppert's algorithm: (a) Phase 1 segment recovery via midpoint splitting until all PSLG segments are present as triangulation edges. (b) Phase 2 bad-triangle refinement: collect triangles with min angle < threshold or area > max_area, sort deterministically (worst-first, canonical tie-break), insert circumcenter unless it encroaches a segment (diametral-circle test), in which case split the encroached segment instead. Termination bound enforced (rejects min_angle > 20.7 deg unless opt-in). max_steiner hard cap. Duplicate-circumcenter guard with centroid perturbation fallback. erify_refined_mesh helper. Registered delaunay_refine_2 OpManifest (Scalar/Wasm, BitExact, TopologyGuaranteed, ColdBounded, D2, Implemented).
3. MEASURED RESULTS    - cargo test --lib delaunay_refine:: -> 11 passed, 0 failed, 0 warnings. Full CG suite: 1389 passed, 0 failed.
4. WHERE I NEED THE HUMAN - none blocking.
5. NEXT STEP           - P13.3 (Optimal fixed-vertex triangulation objectives).


### 2026-07-07 - P13.3 implemented (Optimal fixed-vertex triangulation objectives)
1. STEP + STATUS       - P13.3 (Edge-flip hill-climbing optimisation) - implemented.
2. WHAT WAS BUILT       - New module 	riangulation_opt.rs (~830 lines). Six objectives: MaxMinAngle, MinMaxAngle, MinMaxEdgeRatio, MinMaxRadiusEdge, MaxMinArea, MinMaxAspect. Hill-climbing via edge flips with convex-quad validity check (orientation at all four corners must agree). Deterministic edge evaluation order with canonical tie-breaking. evaluate_objective, optimise_triangulation, optimise_and_evaluate. Registered 	riangulation_optimise OpManifest (Scalar/Wasm, BitExact, ApproximateMetric, ColdBounded, D2, Implemented).
3. MEASURED RESULTS    - cargo test --lib triangulation_opt:: -> 19 passed, 0 failed, 0 warnings. Full CG suite: 1408 passed, 0 failed.
4. WHERE I NEED THE HUMAN - none blocking.
5. NEXT STEP           - P13.4 (Quadtree/octree balanced meshing).

### 2026-07-07 - P13.4 implemented (Quadtree/octree balanced meshing)
1. STEP + STATUS       - P13.4 (Size-field-driven quadtree/octree decomposition with 2:1 balance + conforming extraction) - implemented.
2. WHAT WAS BUILT       - New module quad_octree_mesh.rs (~1759 lines). (a) Quadtree (2-D): flat-Vec nodes with parent/child_idx/first_child links, location-code face-neighbour walk (O(depth), zero-heap stack array, no hashing), size-field-driven refine-to-fixpoint (BFS worklist, max_level + max_cells caps), 2:1 balance pass (subdivide coarser neighbours iterated to fixpoint), conforming triangle extraction via per-cell templates (diagonal split for no-hanging-node leaves, centre-fan for leaves with hanging edge-midpoints, grid-quantised vertex dedup). (b) Octree (3-D): same structure with 8 children/internal node, 6 face-directions, 2:1 balance, hexahedron extraction (8 corner indices, deduplicated vertices), 6-tet Freudenthal cube split (conforming for uniform meshes; documented non-conforming across T-junction faces for graded meshes - use hex mesh + mortar for fully conforming tet). (c) Adapters: size_target_refiner_2d/3d (diameter > target*(1+tol)), size_field_2d_fn (SizeField planar adapter), const_size_fn_3d. Registered quadtree_balanced_mesh_2d + octree_balanced_mesh_3d OpManifests (Scalar/Wasm, BitExact, TopologyGuaranteed, ColdBounded, D2/D3, Implemented).
3. MEASURED RESULTS    - cargo test --lib quad_octree_mesh:: -> 27 passed, 0 failed, 0 warnings. Full CG suite: 1435 passed, 0 failed (up from 1408 = +27 net new). capability_manifests tests green (29 passed).
4. WHERE I NEED THE HUMAN - none blocking. Known limitation: the 3-D tet split is non-conforming across T-junction faces for graded (non-uniform) octrees - documented in the octtree_to_tetrahedra doc comment. Use the hex mesh + mortar methods, or a uniform size, for a fully conforming tet mesh. The 2-D quadtree_to_triangles is always conforming (hanging nodes resolved exactly by centre-fan templates).
5. NEXT STEP           - P13.5 (Advancing-front / Delaunay-based surface mesh generation), then P14.1-P14.6, P13.6-P13.10, P14.7-P14.11.


### 2026-07-07 - P13.5 implemented (Advancing-front surface and volume meshing)
1. STEP + STATUS       - P13.5 (Advancing-front 2-D surface triangulation + 3-D volume tetrahedralisation) - implemented.
2. WHAT WAS BUILT       - New module advancing_front.rs (~1600 lines). (a) 2-D advancing-front surface triangulation (advancing_front_triangulate_2d): closed CCW boundary polyline + target-size function -> triangle mesh. Shortest-edge-first deterministic ordering, equilateral-vertex placement with snap-to-existing, self-crossing guard (proper intersection check via classify_segment_intersection_2), front-joining via remove_front_edge, gap-fill fallback, typed FrontError::Obstruction on stall. (b) 3-D advancing-front volume tetrahedralisation (advancing_front_tetrahedralise_3d): closed orientable surface mesh + target-size function -> tet mesh. Smallest-area-face-first deterministic ordering, inradius-offset apex placement, self-crossing guard (Moller-Trumbore segment-triangle intersection), front-joining via remove_front_face, typed obstruction. Pure advancing-front may not fully close the 3-D front - obstruction is the correct typed failure. Reuses mesh_quality (tri_quality_points, tet_quality_points, SizeField), primitives (orientation_2, Point2, Point3, Orientation), segment_intersection_2 (classify_segment_intersection_2). Registered advancing_front_surface_2d + advancing_front_volume_3d OpManifests (Scalar/Wasm, BitExact, ColdBounded, D2/D3, Implemented) in capability_manifests.rs. AllocationClass::ColdBounded (Tier-2 cold construction), DeterminismClass::BitExact. Also fixed pre-existing build errors in gpu_params.rs (_pad -> dict_pack), mod.rs (missing dict_n_atoms field), attention.rs (field name).
3. MEASURED RESULTS    - cargo test --lib advancing_front:: -> 16 passed, 0 failed, 0 warnings. Coverage: 2-D error paths, 2-D square/pentagon meshing, 2-D CW boundary reversal, 2-D interior points, 2-D determinism, 2-D no-outside-boundary, 3-D error paths, 3-D partial mesh validation, 3-D determinism, 3-D finer mesh, 3-D index range, 3-D no-degenerate. Full CG suite green. capability_manifests tests green.
4. WHERE I NEED THE HUMAN - none blocking. Known limitation: pure advancing-front may not fully close the 3-D front on complex geometries - the algorithm returns a typed FrontError::Obstruction rather than silently producing a broken mesh. This is the correct behaviour for a front-only method; full closure typically requires a combined front + Delaunay back-fill strategy (future work).
5. NEXT STEP           - P14.1-P14.6, P13.6-P13.10, P14.7-P14.11.

### 2026-07-07 - P13.7 implemented (Tetrahedral quality improvement + sliver handling)
1. STEP + STATUS       - P13.7 (Flip/smooth/insert/exude tet-mesh improvement) - implemented.
2. WHAT WAS BUILT       - New module crates/qualia-core-db/src/specialized_libs/computational_geometry/tet_quality_improve.rs (~1820 lines). Four monotonic passes over an existing tet mesh, each preserving the domain boundary and positive tet orientation, each accepting a local op only when it strictly increases the local worst-case score under the selected objective (MinDihedral / RadiusEdge / ScaledJacobian). (a) Flip pass: 2-3 bistellar flip (two tets sharing an interior face -> three tets around the new apex-apex edge) and 3-2 flip (reverse, on interior edges with exactly 3 incident tets); both accepted only on strict local-worst improvement, candidates re-oriented to positive signed volume, degenerate candidates rejected. (b) Smooth pass: optimisation-based vertex smoothing over interior (non-boundary, non-fixed) vertices; deterministic candidate set = Laplacian one-ring centroid, volume-weighted incident-tet centroid, then the 26 unit-cube probe directions x 4 perturbation magnitudes (fractions of local avg incident edge length); accepts the candidate maximising the incident-tets min score only if it beats the current min. (c) Insert pass: Delaunay cavity refinement - locate worst tet, compute its circumcenter, flood-fill the cavity (tets whose circumsphere contains the point), verify star-shapedness (every boundary triangle joined to the new point yields a positive-volume tet), replace cavity with a star of new tets; accepted only on strict worst improvement; Steiner cap. (d) Exude pass: sliver exudation by local perturbation - for each sliver (min_dihedral < threshold) perturb its interior vertices over the fixed 26-direction x 4-magnitude probe set, accept the first perturbation that fixes the sliver (min_dihedral >= threshold), creates no new sliver in the one-ring, and strictly improves the local worst score. Boundary vertices auto-detected (incident to a face in exactly one tet) and always pinned; caller may supply extra fixed vertices. Determinism: worst-first processing with canonical tie-break (lowest index); fixed probe-direction array; no RNG. Public API: improve_tet_mesh(vertices, tets, fixed_vertices, options) -> TetImproveResult { vertices, tets, stats_before, stats_after, flips/smooths/inserts/exudes_applied, passes_run }; verify_improvement helper. Registered tet_quality_improve OpManifest (Scalar/Wasm, BitExact, TopologyGuaranteed, ColdBounded, D3, Implemented) in capability_manifests.rs. Tier-2 cold construction.
3. MEASURED RESULTS    - cargo test --lib -p qualia-core-db tet_quality_improve:: -> 17 passed, 0 failed, 0 warnings from the module. Coverage: degenerate-input rejection; inverted-input-tet rejection; regular tet unchanged (no-op); all output tets positively oriented; monotonic min-dihedral improvement on a flat diamond; boundary + caller-fixed vertices preserved; flip pass preserves validity + monotonicity; 2-3 flip fires iff beneficial (self-verifying over 7 apex heights on a triangular bipyramid - computes both splits' quality and asserts the flip decision matches); smooth pass never inverts; insert pass never inverts; exude pass on a single-tet sliver (all-boundary) correctly refuses to move boundary vertices; exude on a 12-tet cube-with-interior-vertex sliver cluster removes slivers without inverting or regressing min dihedral, cube corners preserved; determinism (bit-identical output across runs); RadiusEdge objective does not regress max radius-edge; ScaledJacobian objective does not regress min scaled Jacobian; verify_improvement helper; full pipeline no-regression on two-tet diamond. NOT measured this step: full computational_geometry suite + capability_manifests tests - the qualia-core-db lib-test target is currently broken by an UNRELATED change in another instrument's lane (crypto/verifiable_credential.rs:277,282 - Credential missing #[derive(PartialEq)]; flagged in NOTICES.md, not my lane). Once that derive is added I will run the full CG suite + manifests for the regression check. wasm32 build gates: same structural argument as P13.5 (the new module inherits the existing specialized_libs cfg gate and uses only core + super::mesh_quality + super::primitives imports).
4. f WHERE I NEED THE HUMAN - none blocking for P13.7 itself. One cross-lane blocker flagged: crypto/verifiable_credential.rs needs #[derive(PartialEq)] on Credential (or the assert_eq! calls changed to matches!) so the lib-test target compiles and I can run the full CG regression suite. That is the Opus wellfair/accountability lane; I did not touch it per the lane rules.
5. NEXT STEP           - P13.8 (Anisotropic remeshing with crease/feature preservation - metric-space edge targets, BVH projection) next, per Timothy's chosen order (P13.7 -> P13.8 -> P13.9 -> P14.1-P14.6 -> P13.6 + P13.10). Will re-run the full CG suite once the VC derive lands.

### 2026-07-08 - P13.8 implemented (Anisotropic remeshing with crease/feature preservation)
1. STEP + STATUS       - P13.8 (anisotropic remeshing with tagged feature preservation and BVH projection) - implemented.
2. WHAT WAS BUILT       - New module crates/qualia-core-db/src/specialized_libs/computational_geometry/anisotropic_remesh.rs. The public API is caller-buffered: anisotropic_remesh(vertices, triangles, field, fixed_vertices, crease_edges, options, out_vertices, out_triangles) plus required_anisotropic_output_capacity. The implementation is a conservative Tier-2 cold builder: it validates manifold input, refines edges whose metric length exceeds the declared band under AnisotropyField/MetricTensor, preserves fixed vertices and tagged crease endpoints exactly, smooths only unconstrained interior vertices with metric-weighted one-ring relaxation, and projects moved vertices back to the original surface through a BVH-pruned exact closest-triangle query. Added FeatureEdge, AnisotropicRemeshOptions, AnisotropicRemeshReport, and typed AnisotropicRemeshError. Wired pub mod / pub use in computational_geometry/mod.rs and registered anisotropic_remesh in capability_manifests.rs as Scalar+Wasm, BitExact, TopologyGuaranteed, ColdBounded, D3, Implemented.
3. MEASURED RESULTS    - cargo test -p qualia-core-db --lib anisotropic_remesh:: -> 8 passed, 0 failed. Coverage: invalid options, indefinite metric fail-closed, caller output buffer fail-closed, metric-long edges split until the max metric edge band is met on the fixture, fixed corner + crease endpoint preservation, BVH projection path exercised and planar projection preserved, deterministic output, and valid triangle quality after refinement. cargo test -p qualia-core-db --lib capability_manifests:: -> 29 passed, 0 failed. cargo test -p qualia-core-db --lib computational_geometry:: -> 1476 passed, 0 failed. Not measured this step: wasm32 build gate and GPU differential; P13.8 has no GPU path and inherits the existing specialized_libs wasm-scientific structure, but a wasm check remains a later cross-target gate.
4. WHERE I NEED THE HUMAN - none blocking. Honest scope fence: this is a feature-preserving anisotropic refinement/relaxation pass, not a full research-grade anisotropic collapse/flip optimizer. Tagged corners/crease endpoints are preserved exactly; crease curves are not yet exported as a propagated feature-tag stream in the output. Full anisotropic collapse/flip and richer feature-tag output can be a future P13.8 refinement or P13.10 certificate/encoding task.
5. NEXT STEP           - P13.9 (Quadrilateral/hexahedral and mixed-cell topology foundation), then P14.1-P14.6 and the remaining P13.6/P13.10 certificate/reconstruction work per the chosen order.

### 2026-07-08 - P13.6/P13.9/P13.10/P14-P19 implemented (late computational-geometry batch)
1. STEP + STATUS       - Implemented the remaining late computational-geometry batch as scalar reference modules and integration descriptors. Status is **implemented**, not verified/done: acceptance gates, GPU differential, wasm cross-target gates, and full evidence-level certification remain separate.
2. WHAT WAS BUILT       - Added `mixed_cell_topology.rs` (P13.9 POD mixed cells, validation, conversion, boundary extraction), `screened_poisson.rs` (P13.6 screened-grid oriented-normal reconstruction), `fem_certificate.rs` (P13.10 canonical FEM certificate encode/decode with CRC), `ddg_operators.rs` additions (P14 chain boundaries, normals/gradients, curvature, cotangent Laplacian, Poisson/heat/MCF, graph geodesics, harmonic disk parameterization, Hodge summary, parallel transport, DDG section encoding), `deterministic_geometry.rs` (P15 scan/reduce, nearest-pair, all-nearest, kNN, EMST, greedy spanner), `motion_planning.rs` (P16 SE(2), polygon collision, visibility graph, seeded roadmap), `math_geometry.rs` (P17 affine frames, tetra barycentrics, quaternion normalization/SLERP, translation registration, AABB separating plane), `parametric_cad.rs` (P18 Bezier/B-spline/NURBS, tensor surface, tube generation, shape distances), and `geometry_integration.rs` (P19 schema/operation/conformance/closeout descriptors). Wired all modules through `computational_geometry/mod.rs` and registered the new manifest entries as Scalar/Wasm, BitExact, ColdBounded reference implementations.
3. MEASURED RESULTS    - Pending final batch test run after implementation, per Timothy's direction to test only after the whole batch is done.
4. WHERE I NEED THE HUMAN - none blocking for the implementation pass. Scope fence: these are compact scalar reference implementations for the planned families, not final verified high-performance/GPU-certified implementations.
5. NEXT STEP           - Run `cargo test -p qualia-core-db --lib` and fix regressions until the lib target is green or a cross-lane blocker is identified.
