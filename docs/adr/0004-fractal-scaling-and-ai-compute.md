...
Rather than increasing the 512MB limit and constructing a monolithic 64GB heap (which re-introduces garbage collection pauses, pointer chasing, and lock-contention), the daemon will dynamically detect hardware and spin up a **Assembly of independent 512MB Cells**. 
...
### 4. Decision: The Sleep-Cycle Assembly (Decentralized AI Compute)
To utilize spare capacity without draining active resources, the daemon implements a **Sleep-Cycle Assembly**.
