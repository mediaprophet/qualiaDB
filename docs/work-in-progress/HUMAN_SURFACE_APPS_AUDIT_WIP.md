...
| **Directory / address book** | Humans-first address book + agreement slots | **WORKING** (findable) · re-UAT Capt | Tauri: `list_directory`, `search_directory`; palette **Directory**; Talk → People shows Directory by default; QApps → Directory (Active) launches Talk/People. No new top-level IA name. | Pane invokes need Native host; empty agreements **held / empty** | `command_palette.rs`; `shell_html.rs` PALETTE; `directory.rs` who_kind; `directory_pane.rs`; `relations/people.rs` | Capt re-UAT: cold Desktop → Ctrl+K → `dir` → Enter; confirm humans first, org/tool chrome, no “unavailable” |
...
|---------|------------|---------------|
| **Directory / address book** | **Was** toggle-only; now palette + Talk/People + QApps Active (2026-09-12 findable PR) | `search_directory`; `DirectoryPane`; palette id `directory` |
