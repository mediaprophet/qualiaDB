...
use vibe::{
    ...
    parse_program, DiagCode, Env, LocalHost, Value,
};
...
#[test]
fn language_version() {
    assert_eq!(vibe::LANGUAGE_VERSION, "vibe-0.1");
}
...
fn g2_tick_time_effect_fn() {
    ...
    assert_eq!(
        v.as_i64(),
        Some(1_000_000_000),
        "LocalHost returns deterministic epoch"
    );
}
...
fn x6_time_now_evaluates() {
    ...
    let result = vibe::eval_function(&program, "main", vec![], &mut host, &mut env)
        .expect("x6 fixture should evaluate");
    // LocalHost returns Instant with secs=1_000_000_000, projected to i64.
    assert_eq!(result, vibe::Value::I64(1_000_000_000));
}
...
use vibe::bytecode::{compile, compile_expr, decode_chunk, encode_chunk, Vm};
...
fn bc1_cell_arithmetic() {
    ...
    let mut host = LocalHost::default();
    let mut vm = Vm::new(&chunk, &mut host, vibe::Budget::default());
...
fn bc10_encode_decode_execute() {
    ...
    let mut host = LocalHost::default();
    let mut vm = Vm::new(&decoded, &mut host, vibe::Budget::default());
    vm.run().expect("preamble");
