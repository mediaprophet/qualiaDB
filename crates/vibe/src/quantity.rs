...
pub fn lookup_unit(symbol: &str) -> Option<Unit> {
    match symbol {
        ...
        "Hz" => Some(Unit::new("Hz", Dimension::frequency(), 1.0)),
        "kHz" => Some(Unit::new("kHz", Dimension::frequency(), 1_000.0)),
        "MHz" => Some(Unit::new("MHz", Dimension::frequency(), 1_000_000.0)),
        "GHz" => Some(Unit::new("GHz", Dimension::frequency(), 1_000_000_000.0)),
        "fps" => Some(Unit::new("fps", Dimension::frequency(), 1.0)),
        "nm" => Some(Unit::new("nm", Dimension::length(), 1e-9)),
        "deg" => Some(Unit::new("deg", Dimension::dimensionless(), 1.0)),
        "rad" => Some(Unit::new("rad", Dimension::dimensionless(), 1.0)),
        "%" => Some(Unit::new("%", Dimension::dimensionless(), 0.01)),
        "m/s" => Some(Unit::new("m/s", Dimension::velocity(), 1.0)),
        _ => None,
    }
}

/// Canonical unit identifier used at check time (T73 / UnitId).
...
impl UnitId {
    ...
    pub fn compatible(&self, other: &Self) -> bool {
        match (lookup_unit(&self.symbol), lookup_unit(&other.symbol)) {
            (Some(a), Some(b)) => a.dimension == b.dimension,
            _ => self.symbol == other.symbol,
        }
    }
}

/// Linear tween of two same-unit scalars. `t` is clamped to `[0, 1]`.
pub fn tween(a: f64, a_unit: &str, b: f64, b_unit: &str, t: f64) -> Result<f64, String> {
...
mod tests {
    ...
    #[test]
    fn tween_same_unit() {
        assert_eq!(tween(0.0, "m", 10.0, "m", 0.5).unwrap(), 5.0);
    }

    #[test]
    fn tween_compatible_length_units() {
        assert!(tween(1.0, "m", 2.0, "km", 0.5).is_ok());
    }

    #[test]
    fn tween_rejects_dimension_mismatch() {
        assert!(tween(1.0, "m", 2.0, "s", 0.5).is_err());
    }
}
