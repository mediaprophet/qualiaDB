...
mod tests {
    ...
    fn vc5_prohibition_breach_completes_under_1_microsecond() {
        ...
        //
        // We measure the best-of-N rather than a single shot, because
        // single-shot timing assertions are flaky under CI / OS scheduling
        // jitter. The best-of-N approach is standard for microbenchmark-style
        // assertions: the hardware capability is what we're testing, and
        // taking the minimum removes OS-level noise.
        const MEASUREMENTS: usize = 32;
        ...
        for _ in 0..MEASUREMENTS {
            ...
            if elapsed.as_nanos() < best_ns {
                best_ns = elapsed.as_nanos();
            }
        ...
        // The best-of-N trigger time must be < 1µs (spec requirement).
    ...
    fn vc5_global_halt_revokes_all_agents_under_1_microsecond() {
        ...
        //
        // Best-of-N measurement to remove OS scheduling jitter.
