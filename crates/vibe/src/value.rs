...
pub enum Value {
    ...
    Enum(EnumValue),
    /// Non-capturing lambda (body is AST; eval uses a fresh env of params only).
    Lambda {
        params: Vec<String>,
...
impl std::fmt::Display for Duration {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if self.secs == 0 && self.nanos % 1_000_000 == 0 {
...
impl std::fmt::Display for Value {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Value::Prefixed(p, n) => write!(f, "{p}:{n}"),
            ...
            Value::Record(map) => {
                if let Some(Value::String(hex)) = map.get("hex") {
                    if hex.starts_with('#') {
                        if let Some(Value::String(space)) = map.get("space") {
                            if space == "srgb" {
                                return write!(f, "{hex}");
                            }
                        }
            ...
            Value::Quantity(q) => {
                if q.value.fract() == 0.0 && q.value.abs() < 1e15 {
                    write!(f, "{}{}", q.value as i64, q.unit)
                } else {
                    write!(f, "{}{}", q.value, q.unit)
                }
            }
            Value::Duration(d) => write!(f, "{d}"),
            Value::Instant(i) => {
                if i.secs == 0 && i.nanos % 1_000_000 == 0 {
                    write!(f, "{}ms", i.nanos / 1_000_000)
                } else if i.nanos == 0 {
                    write!(f, "{}s", i.secs)
                } else {
                    write!(f, "{}.{:09}s", i.secs, i.nanos)
                }
            }
            Value::Ok(v) => write!(f, "Ok({v})"),
            Value::Err(v) => write!(f, "Err({v})"),
            Value::Receipt => write!(f, "receipt"),
            other => write!(f, "{other:?}"),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn display_duration_as_ms() {
        assert_eq!(
            format!("{}", Value::Duration(Duration::from_nanos(500_000_000))),
            "500ms"
        );
    }

    #[test]
    fn display_quantity_keeps_unit() {
        assert_eq!(
            format!("{}", Value::Quantity(Quantity::new(500.0, "ms"))),
            "500ms"
        );
    }

    #[test]
    fn display_srgb_color_as_hex() {
        let mut rec = BTreeMap::new();
        rec.insert("space".into(), Value::String("srgb".into()));
        rec.insert("hex".into(), Value::String("#ff2d95".into()));
        rec.insert("r".into(), Value::U64(255));
        rec.insert("g".into(), Value::U64(45));
        rec.insert("b".into(), Value::U64(149));
        rec.insert("a".into(), Value::U64(255));
        assert_eq!(format!("{}", Value::Record(rec)), "#ff2d95");
    }
}
