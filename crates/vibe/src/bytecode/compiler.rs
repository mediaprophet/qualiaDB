...
//!
//! Compiles a checked `vibe::ast::Program` into a `Chunk`. The
...
impl Compiler {
    ...
    fn compile_expr(&mut self, e: &Expr) -> Result<(), CompileError> {
        match &e.kind {
            ...
            ExprKind::GraphQuery { .. } | ExprKind::ModalLogic { .. } => {
                Err(CompileError::Unsupported(
                    "graph query / modal logic in pure bytecode VM; use AST evaluator",
                ))
            }
            ExprKind::Lambda { .. } => Err(CompileError::Unsupported(
                "lambda in bytecode VM; use AST evaluator",
            )),
            ExprKind::Tween { .. } => Err(CompileError::Unsupported(
                "tween in bytecode VM; use AST evaluator",
            )),
    ...
    fn compile_literal(&mut self, lit: &Literal) -> Result<(), CompileError> {
        match lit {
            ...
            Literal::Quantity { value, unit } => {
                let idx = self.add_string(unit)?;
                self.emit_op(Op::PushQuantity)?;
                self.emit_u64(*value)?;
                self.emit_u16(idx)
            }
            Literal::String(s) => {
    ...
    fn compile_pattern_check(&mut self, pat: &Pattern) -> Result<(), CompileError> {
        match pat {
            ...
            | Pattern::List(_)
            | Pattern::Constructor { .. } => {
                // Variant/destructuring pattern matching requires runtime type info.
...
mod tests {
    ...
    fn compile_quantity_keeps_unit_opcode() {
        ...
        let has_unit = chunk.constants.iter().any(|c| match c {
            Const::String(s) => s == "ms",
            _ => false,
        });
        assert!(has_unit, "unit string must stay in the constant pool");
    }
}
