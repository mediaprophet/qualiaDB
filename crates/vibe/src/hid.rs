...
pub enum EventKind {
    ...
    Assistive,
    /// Gamepad / game controller buttons, thumbsticks, and triggers.
    Gamepad,
    /// 6-DoF Spatial Head pose (position + orientation quaternion).
    SpatialHead,
    /// 26-joint articulated Spatial Hand skeleton and gesture confidence.
    SpatialHand,
    /// Eye gaze ray, pupilometry, and vergence tracking.
    SpatialGaze,
    /// MIDI controller event (note on/off, control change, pitch bend).
    Midi,
    /// Outbound/Inbound haptic actuation and vibrotactile feedback.
    Haptic,
}
...
impl EventKind {
    ...
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::SpatialHead => "spatial_head",
            Self::SpatialHand => "spatial_hand",
            Self::SpatialGaze => "spatial_gaze",
            Self::Midi => "midi",
            Self::Haptic => "haptic",
        }
    }
    ...
    pub fn from_str(s: &str) -> Option<Self> {
        match s {
            ...
            "spatial_head" => Some(Self::SpatialHead),
            "spatial_hand" => Some(Self::SpatialHand),
            "spatial_gaze" => Some(Self::SpatialGaze),
            "midi" => Some(Self::Midi),
...
impl InboundEvent {
    ...
    /// Create a Gamepad event (buttons + axes).
    pub fn gamepad(timestamp_ns: u64, source_id: &str, buttons: u32, axes: &[f64]) -> Self {
        let axis_values: Vec<Value> = axes.iter().map(|&a| Value::F64(a)).collect();
        Self::new(
            timestamp_ns,
            source_id,
            EventKind::Gamepad,
            EventPayload::inline_pairs(&[
                ("buttons", Value::U64(buttons as u64)),
                ("axes", Value::List(axis_values)),
            ]),
        )
    }

    /// Create a 6-DoF Spatial Head pose event.
    pub fn spatial_head(timestamp_ns: u64, source_id: &str, pos: [f64; 3], ori: [f64; 4]) -> Self {
        Self::new(
            timestamp_ns,
            source_id,
            EventKind::SpatialHead,
            EventPayload::inline_pairs(&[
                ("x", Value::F64(pos[0])),
                ("y", Value::F64(pos[1])),
                ("z", Value::F64(pos[2])),
    ...
    /// Create a 26-joint Spatial Hand skeleton tracking event.
    pub fn spatial_hand(
        timestamp_ns: u64,
        source_id: &str,
        is_left: bool,
        joint_asset_iri: &str,
        joint_asset_hash: u64,
        pinch_confidence: f64,
    ) -> Self {
        let mut e = Self::new(
            timestamp_ns,
            source_id,
            EventKind::SpatialHand,
            EventPayload::fat_buffer(joint_asset_iri, joint_asset_hash),
        );
        if let EventPayload::Inline(ref mut m) = e.payload {
            m.insert("is_left".into(), Value::Bool(is_left));
            m.insert("pinch_confidence".into(), Value::F64(pinch_confidence));
        }
        e
    }

    /// Create a Spatial Gaze tracking ray event.
    pub fn spatial_gaze(
        timestamp_ns: u64,
        source_id: &str,
        origin: [f64; 3],
        ...
        Self::new(
            timestamp_ns,
            source_id,
            EventKind::SpatialGaze,
            EventPayload::inline_pairs(&[
                ("ox", Value::F64(origin[0])),
                ("oy", Value::F64(origin[1])),
                ("oz", Value::F64(origin[2])),
                ("dx", Value::F64(direction[0])),
                ("dy", Value::F64(direction[1])),
                ("dz", Value::F64(direction[2])),
                ("pupil_diameter_mm", Value::F64(pupil_dia_mm)),
            ]),
        )
    }

    /// Create a MIDI controller message event.
    pub fn midi(timestamp_ns: u64, source_id: &str, status: u8, data1: u8, data2: u8) -> Self {
        Self::new(
            timestamp_ns,
            source_id,
            EventKind::Midi,
            EventPayload::inline_pairs(&[
                ...
                ("data1", Value::U64(data1 as u64)),
                ("data2", Value::U64(data2 as u64)),
            ]),
        )
    }

    /// Create a Haptic actuation event.
    pub fn haptic(
        timestamp_ns: u64,
        source_id: &str,
        pattern: &str,
        ...
        Self::new(
            ...
            EventPayload::inline_pairs(&[
                ...
                ("intensity", Value::F64(intensity.clamp(0.0, 1.0))),
...
mod tests {
    ...
    #[test]
    fn inbound_event_gamepad_and_spatial() {
        let g = InboundEvent::gamepad(1000, "gamepad:0", 0b0011, &[0.5, -0.5]);
        assert_eq!(g.kind, EventKind::Gamepad);
        assert!(g.payload.is_inline());
        ...
        assert_eq!(m.kind, EventKind::Midi);

        let hap = InboundEvent::haptic(5000, "haptic:0", "buzz", 0.8, 150.0);
        assert_eq!(hap.kind, EventKind::Haptic);
    }
