...
fn query_item(item: &Item, pattern: &QueryPattern, results: &mut Vec<PolicyViolation>) {
    match item {
        ...
        Item::Cell(c) => {
            query_expr(&c.expr, pattern, results);
            if let Some(w) = &c.when {
                query_expr(w, pattern, results);
            }
        }
        Item::Present(p) => {
        ...
        Item::Bind(b) => {
            query_expr(&b.left, pattern, results);
            query_expr(&b.right, pattern, results);
            if let Some((lo, hi)) = &b.clamp {
                query_expr(lo, pattern, results);
                query_expr(hi, pattern, results);
            }
        }
    }
}
