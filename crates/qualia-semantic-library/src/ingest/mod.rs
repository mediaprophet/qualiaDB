...
impl Default for IngestOptions {
    fn default() -> Self {
        IngestOptions { out_dir: PathBuf::from("library"), recursive: true, skip_existing: true }
    }
}
...
pub fn ingest_file(path: &Path, opts: &IngestOptions) -> anyhow::Result<IngestResult> {
    ...
    let target = opts.out_dir.join(format!("{doc_id}.{}", crate::container::HMC_EXTENSION));

    if opts.skip_existing && target.exists() {
        return Ok(IngestResult { container_path: target, doc_id, skipped: true, notes: vec![] });
    }
    ...
    w.add_derived(AssetKind::Text, "document.txt", "text/plain", extracted.text.into_bytes());
    w.add_derived(AssetKind::Html, "document.html", "text/html", extracted.html.into_bytes());

    let jsonl = chunk::chunks_to_jsonl(&chunks);
    w.add_derived(AssetKind::Chunks, "chunks.jsonl", "application/jsonl", jsonl.into_bytes());

    let ttl = cml::build_cml(w.manifest(), &chunks);
    w.add_derived(AssetKind::Cml, "document.cml.ttl", "text/turtle", ttl.into_bytes());
    ...
    Ok(IngestResult { container_path, doc_id, skipped: false, notes: extracted.notes })
}
...
pub fn ingest_path(path: &Path, opts: &IngestOptions) -> anyhow::Result<Vec<IngestResult>> {
    ...
    for entry in walker.into_iter().filter_map(|e| e.ok()) {
        ...
        let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| ingest_file(p, opts)));
