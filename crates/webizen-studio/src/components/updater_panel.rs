...
const TITLE_STYLE: &str =
    "font-size: 1.1rem; font-weight: 700; color: var(--qualia-text); margin: 0;";
const DESC_STYLE: &str =
    "font-size: 0.85rem; color: var(--qualia-text-muted); line-height: 1.5; margin: 0;";
...
const PROGRESS_BAR: &str =
    "height: 100%; background: var(--qualia-brand); transition: width 0.2s ease;";
...
pub fn UpdaterPanel() -> Element {
    ...
    let check_update = move |_| {
        ...
        spawn(async move {
            let res =
                invoke_tauri_json::<Option<String>>("updater_check", serde_json::json!({})).await;
            match res {
    ...
    let download_update = move |_| {
        ...
        spawn(async move {
            ...
            let cb = Closure::wrap(Box::new(move |event_val: js_sys::Object| {
                if let Ok(event) =
                    serde_wasm_bindgen::from_value::<TauriEventPayload>(event_val.into())
                {
                    state.set(UpdaterState::Downloading {
            ...
            let res =
                invoke_tauri_json::<()>("updater_download_and_install", serde_json::json!({}))
                    .await;
            if let Err(e) = res {
