...
#[cfg(target_arch = "wasm32")]
pub async fn record_transparency_cc(
    credential_id: &str,
    ...
    for (k, v) in [
        ...
        ("informedAuthorityDid", informed_authority_did),
        ("purpose", purpose),
    ] {
        js_sys::Reflect::set(&args, &k.into(), &wasm_bindgen::JsValue::from_str(v))
            .map_err(|_| "args".to_string())?;
    }
    tauri_invoke("wellfair_record_transparency_cc", args.into())
        .await
        .map_err(|e| format!("{e:?}"))?;
    Ok(())
}
...
pub async fn record_disclosure(
    ...
    if let Some(d) = acting_delegate_did {
        js_sys::Reflect::set(
            &args,
            &"actingDelegateDid".into(),
            &wasm_bindgen::JsValue::from_str(&d),
        )
        .map_err(|_| "args".to_string())?;
    }
    ...
    let js = tauri_invoke("wellfair_record_disclosure", args.into())
        .await
        .map_err(|e| format!("{e:?}"))?;
    let json = js
        .as_string()
...
pub async fn assess_duty_of_inquiry(duty_json: &str, conduct_json: &str) -> Result<String, String> {
    ...
    js_sys::Reflect::set(
        &args,
        &"dutyJson".into(),
        &wasm_bindgen::JsValue::from_str(duty_json),
    )
    .map_err(|_| "args".to_string())?;
    js_sys::Reflect::set(
        &args,
        &"conductJson".into(),
        &wasm_bindgen::JsValue::from_str(conduct_json),
    )
    .map_err(|_| "args".to_string())?;
    let js = tauri_invoke("wellfair_assess_duty_of_inquiry", args.into())
        .await
        .map_err(|e| format!("{e:?}"))?;
    let json = js
        .as_string()
...
#[cfg(not(target_arch = "wasm32"))]
pub async fn assess_duty_of_inquiry(
    _duty_json: &str,
    _conduct_json: &str,
