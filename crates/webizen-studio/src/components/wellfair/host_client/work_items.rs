...
pub async fn add_work_item(
    ...
    js_sys::Reflect::set(
        &args,
        &"itemType".into(),
        &wasm_bindgen::JsValue::from_str(item_type),
    )
    .map_err(|_| "failed to build invoke args".to_string())?;
    js_sys::Reflect::set(
        &args,
    ...
    let out = js
        .as_string()
        .ok_or_else(|| "work item response not JSON".to_string())?;
...
#[cfg(target_arch = "wasm32")]
pub async fn add_work_item_status(
    work_item_id: &str,
    status: &str,
) -> Result<HealthRecordDto, String> {
    let args = js_sys::Object::new();
    js_sys::Reflect::set(
        &args,
        &"workItemId".into(),
        &wasm_bindgen::JsValue::from_str(work_item_id),
    )
    .map_err(|_| "failed to build invoke args".to_string())?;
    js_sys::Reflect::set(
        &args,
    ...
    let out = js
        .as_string()
        .ok_or_else(|| "status response not JSON".to_string())?;
    serde_json::from_str(&out).map_err(|e| e.to_string())
}
...
pub async fn fetch_work_item_board(project_id: &str) -> Result<Vec<BoardColumnDto>, String> {
    ...
    let json = js
        .as_string()
        .ok_or_else(|| "board response not JSON".to_string())?;
