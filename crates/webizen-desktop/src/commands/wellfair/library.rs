...
// â”€â”€ Hypermedia asset library: ingest a document â†’ find it by meaning â”€â”€

/// Ingest a text document into the library (derive topics + searchable text; guardianship flagâ†’notify).
#[command]
...
pub fn wellfair_search_library_text(app: AppHandle, query: String) -> Result<String, String> {
    ...
    state.0.execute_sync(move |guard| {
        let host = guard
            .as_ref()
            .ok_or_else(|| "Host API not initialized â€” unlock Sanctuary vault first".to_string())?;
