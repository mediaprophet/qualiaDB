...
/// Build a VibeScript payload for a tool's action.
///
/// This is a free function rather than a trait method so that the
/// `Tool` trait remains dyn-compatible (`Box<dyn Tool>`). The generic
/// parameter `P` prevents the method from being object-safe.
pub fn build_payload<P>(
    tool: &dyn Tool,
