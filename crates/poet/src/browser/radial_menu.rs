...
/// Wire the WASM/desktop human gesture path (right-click + long-press).
///
/// Capt A4 on `cfac542` still saw the browser menu: document `contextmenu`
/// alone did not own Dual Studio / live viewport hits. Gesture policy and
/// surface binds live in `radial_gesture`.
pub fn wire_radial_menu(document: &Document) {
    super::radial_gesture::wire_human_gestures(document);
}

/// Re-bind after canvas rerender so Dual Studio / viewport keep the wheel.
pub fn bind_live_surfaces(document: &Document) {
    super::radial_gesture::bind_live_surfaces(document);
}
...
mod tests {
    ...
    fn context_target_prefers_element_over_empty() {
        ...
        assert!(super::super::radial_gesture::event_element_from_target(None).is_none());
    }
