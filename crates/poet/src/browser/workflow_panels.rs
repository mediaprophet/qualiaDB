...
pub fn build_publication_workflow_view(document: &Document) -> Element {
    ...
    note.set_text_content(Some(
        "\u{1F4A1} Steps 1\u{2013}2 are live (via File \u{203A} Save As\u{2026}). \
         Steps 3\u{2013}8 are present \u{2014} constituency tracking, consent checking, \
         tombstone pruning, credits generation, distribution export, and metadata \
         stripping are engine wiring pending. See SAVE_ARCHITECTURE.md.",
    ));
    wrapper.append_child(&note).unwrap();
