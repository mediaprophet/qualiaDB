//! Vitals — vital sign time-series chart (§2, P1).
//!
...
const VITALS: &[(&str, &str, &str, &str, &str)] = &[
    (
        "Heart Rate",
        "72 bpm",
        "60-100",
    ...
    ("SpO2", "98%", "95-100", "2026-08-18 08:00", "normal"),
    ("Weight", "78 kg", "—", "2026-08-17 07:00", "normal"),
    (
        "Heart Rate",
        "68 bpm",
        "60-100",
...
pub fn build_vitals_view(document: &Document) -> Element {
    ...
    for label in &["+ Log Vital", "Export CSV", "Chart View"] {
    ...
    let table = make_table(
        document,
        &["Metric", "Value", "Ref Range", "Timestamp", "Flag"],
    );
    let tbody = document.create_element("tbody").unwrap();
    for (metric, value, ref_range, ts, flag) in VITALS {
        let tr = document.create_element("tr").unwrap();
        for (i, val) in [metric, value, ref_range, ts, flag].iter().enumerate() {
    ...
    footer.set_text_content(Some(
        "\u{26A0} Mock data \u{2014} vitals require HW-1 vital sign engine + time-series chart rendering.",
    ));
