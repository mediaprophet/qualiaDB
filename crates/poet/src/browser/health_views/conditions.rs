//! Conditions — condition and allergy list (§2, P0).
//!
...
const CONDITIONS: &[(&str, &str, &str, &str, &str, &str)] = &[
    (
        "Hypothyroidism",
        "2024-03-15",
        "active",
        "Dr. Chen",
        "Levothyroxine 50mcg/day",
        "ICD-10 E03.9",
    ),
    (
...
const ALLERGIES: &[(&str, &str, &str, &str)] = &[
    ("Penicillin", "rash", "severe", "Dr. Chen"),
    ("Shellfish", "anaphylaxis", "severe", "Self-reported"),
    ("Pollen", "rhinitis", "mild", "Self-reported"),
];
...
pub fn build_conditions_view(document: &Document) -> Element {
    ...
    content
        .append_child(&build_conditions_tab(document, "active"))
        .unwrap();

    for (i, (tab_id, _)) in TABS.iter().enumerate().skip(1) {
        let panel = build_conditions_tab(document, tab_id);
        if i > 0 {
    ...
    footer.set_text_content(Some(
        "\u{26A0} Mock data \u{2014} conditions require wellfare-core/conditions.rs engine.",
    ));
...
fn build_tab_bar(document: &Document) -> Element {
    ...
    for (i, (tab_id, tab_label)) in TABS.iter().enumerate() {
        ...
        tab.set_attribute("data-conditions-tab", tab_id).unwrap();
...
fn build_conditions_tab(document: &Document, filter: &str) -> Element {
    let panel = document.create_element("div").unwrap();
    panel
        .set_attribute("data-conditions-panel", filter)
        .unwrap();
    ...
    match filter {
        "allergies" => {
            let table = make_table(
                document,
                &["Allergen", "Reaction", "Severity", "Reported By"],
            );
            let tbody = document.create_element("tbody").unwrap();
            for (allergen, reaction, severity, reported_by) in ALLERGIES {
                let tr = document.create_element("tr").unwrap();
                for (i, val) in [allergen, reaction, severity, reported_by]
                    .iter()
                    .enumerate()
                {
                    ...
                    if i == 2 {
                        let color = match **val {
                            "severe" => "rgba(255, 100, 100, 0.8)",
                            "moderate" => "rgba(255, 165, 0, 0.8)",
                            "mild" => "rgba(100, 200, 100, 0.8)",
                            _ => "var(--text-primary)",
                        };
        ...
        _ => {
            ...
            let tbody = document.create_element("tbody").unwrap();
            for (name, date, status, clinician, treatment, code) in CONDITIONS {
                ...
                let tr = document.create_element("tr").unwrap();
                for (i, val) in [name, date, status, clinician, treatment, code]
                    .iter()
                    .enumerate()
                {
                    let td = document.create_element("td").unwrap();
    ...
    btn.set_text_content(Some("+ Add Condition"));
