//! Procedures — medical procedure records (§2, P1).
//!
...
const PROCEDURES: &[(&str, &str, &str, &str, &str, &str)] = &[
    (
        "PRC-001",
        "Appendectomy",
        "2010-05-15",
...
pub fn build_procedures_view(document: &Document) -> Element {
    ...
    let table = make_table(
        document,
        &["ID", "Procedure", "Date", "Facility", "Clinician", "Status"],
    );
    let tbody = document.create_element("tbody").unwrap();
    for (id, name, date, facility, clinician, status) in PROCEDURES {
        let tr = document.create_element("tr").unwrap();
        for (i, val) in [id, name, date, facility, clinician, status]
            .iter()
            .enumerate()
        {
    ...
    footer.set_text_content(Some(
        "\u{26A0} Mock data \u{2014} procedures require HW-3 procedure engine.",
    ));
