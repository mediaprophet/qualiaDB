...
pub fn project_documents(records: &[HealthRecord]) -> Vec<DocumentItem> {
    records
        ...
        .map(|r| {
            let title = r.field_text("title").or_else(|| r.field_text("name")).unwrap_or_else(|| r.title.clone());
            let doc_type = DocumentType::parse(&r.field_text("doc_type").or_else(|| r.field_text("type")).unwrap_or_default());
            let author = r.field_text("author");
            let facility = r.field_text("facility");
            let date = r.field_text("date").or_else(|| r.field_text("occurred_at")).unwrap_or_else(|| "Undated".into());
            ...
            let pipeline_status = r.field_text("pipeline_status").unwrap_or_else(|| "extracted_text_only".into());
...
pub fn build_document_payload(
    ...
    fields.insert("title".into(), serde_json::Value::String(clean_title.clone()));
    fields.insert("name".into(), serde_json::Value::String(clean_title.clone()));
    fields.insert("doc_type".into(), serde_json::Value::String(doc_type.trim().to_string()));
    fields.insert("type".into(), serde_json::Value::String(doc_type.trim().to_string()));
    fields.insert("date".into(), serde_json::Value::String(date.trim().to_string()));
    fields.insert("occurred_at".into(), serde_json::Value::String(date.trim().to_string()));

    if let Some(auth) = author.filter(|s| !s.trim().is_empty()) {
        fields.insert("author".into(), serde_json::Value::String(auth.trim().to_string()));
    }
    if let Some(fac) = facility.filter(|s| !s.trim().is_empty()) {
        fields.insert("facility".into(), serde_json::Value::String(fac.trim().to_string()));
    }
    ...
    fields.insert("pipeline_status".into(), serde_json::Value::String(pipeline_status.into()));
    fields.insert("source_uri".into(), serde_json::Value::String(source_uri.trim().to_string()));
...
pub fn project_reports(records: &[HealthRecord]) -> Vec<ReportItem> {
    records
        ...
        .map(|r| {
            let title = r.field_text("title").or_else(|| r.field_text("name")).unwrap_or_else(|| r.title.clone());
            let report_type = ReportType::parse(&r.field_text("report_type").or_else(|| r.field_text("type")).unwrap_or_default());
            let author = r.field_text("author");
            let facility = r.field_text("facility");
            let date = r.field_text("date").or_else(|| r.field_text("occurred_at")).unwrap_or_else(|| "Undated".into());
            let findings = r.field_text("findings");
            let recommendations = r.field_text("recommendations");
            let sensitivity = r.field_text("sensitivity").unwrap_or_else(|| "restricted".into());
...
pub fn build_report_payload(
    ...
    fields.insert("title".into(), serde_json::Value::String(clean_title.clone()));
    fields.insert("name".into(), serde_json::Value::String(clean_title.clone()));
    fields.insert("report_type".into(), serde_json::Value::String(report_type.trim().to_string()));
    fields.insert("type".into(), serde_json::Value::String(report_type.trim().to_string()));
    fields.insert("date".into(), serde_json::Value::String(date.trim().to_string()));
    fields.insert("occurred_at".into(), serde_json::Value::String(date.trim().to_string()));
    ...
    if let Some(f) = findings.filter(|s| !s.trim().is_empty()) {
        fields.insert("findings".into(), serde_json::Value::String(f.trim().to_string()));
    }
    if let Some(rec) = recommendations.filter(|s| !s.trim().is_empty()) {
        fields.insert("recommendations".into(), serde_json::Value::String(rec.trim().to_string()));
    }
    fields.insert("sensitivity".into(), serde_json::Value::String(sensitivity.trim().to_string()));
...
mod tests {
    ...
    fn document_type_parsing_and_labels() {
        assert_eq!(DocumentType::parse("discharge_summary"), DocumentType::DischargeSummary);
        assert_eq!(DocumentType::parse("pathology"), DocumentType::PathologyReport);
        assert_eq!(DocumentType::parse("consult_letter"), DocumentType::ConsultationLetter);
        assert_eq!(DocumentType::parse("imaging"), DocumentType::ImagingReport);
        assert_eq!(DocumentType::parse("clinical_note"), DocumentType::ClinicalNote);
        assert_eq!(DocumentType::parse("random"), DocumentType::Other);

        assert_eq!(DocumentType::DischargeSummary.display_label(), "Discharge Summary");
        assert_eq!(DocumentType::PathologyReport.display_label(), "Pathology Report");
    }
    ...
    fn project_documents_extracts_typed_metadata_and_snippet() {
        ...
        let doc1 = items.iter().find(|d| d.title == "Hospital Discharge Summary").unwrap();
        ...
        let doc2 = items.iter().find(|d| d.title == "Histopathology Biopsy").unwrap();
    ...
    fn project_reports_extracts_findings_and_recommendations() {
        ...
        assert_eq!(rep.findings.as_deref(), Some("Sinus rhythm with rare premature ventricular contractions."));
        assert_eq!(rep.recommendations.as_deref(), Some("Continue current antihypertensive therapy. Follow up in 6 months."));
    }
    ...
    fn build_payloads_generate_correct_fields() {
        ...
        assert_eq!(d_fields.get("pipeline_status").unwrap(), "analyzed_and_ingested");
        ...
        assert_eq!(r_fields.get("findings").unwrap(), "Uncomplicated procedure.");
    }
}
