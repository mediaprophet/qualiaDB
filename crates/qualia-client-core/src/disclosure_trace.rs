...
impl DisclosureEvent {
    ...
    pub fn accountable_actor(&self) -> &str {
        self.acting_delegate_did
            .as_deref()
            .unwrap_or(&self.recipient_did)
    }
}
...
pub fn disclosure_chain<'a>(
    ...
    events
        .iter()
        .filter(|e| &e.payload_commitment == commitment)
        .collect()
}
...
pub fn actors_with_access<'a>(
    ...
    {
        for actor in [
            Some(e.recipient_did.as_str()),
            e.acting_delegate_did.as_deref(),
        ]
        .into_iter()
        .flatten()
        {
...
mod tests {
    ...
    fn a_staff_leak_is_attributed_to_the_staffer_not_only_the_authority() {
        ...
        assert_eq!(
            src.recipient_did, "did:wf:mp-smith",
            "under the MP's credential"
        );
        assert_eq!(
            src.accountable_actor(),
    ...
    fn onward_sharing_is_recorded_so_the_chain_is_traced() {
        ...
        let events = vec![
            ...
            ev(
                ...
                Some("did:wf:mp-staffer-jones"),
                [4u8; 16],
                DisclosureKind::OnwardShare {
                    to_did: "did:wf:perpetrator-pep".into(),
                },
            ),
        ];
