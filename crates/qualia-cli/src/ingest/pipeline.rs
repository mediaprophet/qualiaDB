...
impl IncrementalIngestor {
    pub fn new(scratch_dir: &Path, memory_limit: usize) -> Self {
        assert!(
            ...
            "Pipeline execution space breaks 512MB RAM floor constraint."
        );
        Self {
    ...
    pub fn execute_stream_compilation(
        &self,
        input_path: &Path,
        output_path: &Path,
    ) -> Result<(), crate::ingest::IngestError> {
        let wal_path = self
            .execute_stream_to_wal(input_path)
            .map_err(|e| crate::ingest::IngestError::Io(e))?;
        ...
        pool.execute_parallel_cell_resolution(&wal_path, output_path)
            .map_err(|e| crate::ingest::IngestError::Io(e))?;
    ...
    pub fn execute_stream_to_wal(&self, input_path: &Path) -> std::io::Result<PathBuf> {
        ...
        let mut wal_writer =
            BufWriter::with_capacity(INGEST_STREAM_BUFFER_SIZE, File::create(&wal_path)?);
        ...
        while let Ok(bytes_read) = reader.read_until(b'\n', &mut buffer) {
            ...
            if last_print.elapsed().as_millis() >= 200 {
                ...
                let percent = if total_bytes > 0 {
                    (total_bytes_read as f64 / total_bytes as f64) * 100.0
                } else {
                    0.0
                };
                let bytes_left = total_bytes.saturating_sub(total_bytes_read);
                let time_left = if bps > 0.0 {
                    bytes_left as f64 / bps
                } else {
                    0.0
                };
                let est_total_lines = if total_bytes_read > 0 {
                    (lines_processed as f64 * (total_bytes as f64 / total_bytes_read as f64)) as u64
                } else {
                    0
                };
            ...
            if slice.is_empty()
                || slice.starts_with(b"#")
                || slice.iter().all(|b| b.is_ascii_whitespace())
            {
                buffer.clear();
                continue;
            }
    ...
    pub fn build_external_merge_lexicon(
        &self,
        _string_run_paths: &[PathBuf],
        ...
        while wal_reader.read_exact(&mut raw_bytes).is_ok() {
            ...
            let quin = NQuin {
                ...
                parity: NQuin::calculate_parity(
                    raw_quin.hash_subject,
                    raw_quin.hash_predicate,
                    raw_quin.hash_object,
                    raw_quin.hash_context,
                    raw_quin.hash_metadata,
                ),
            };
