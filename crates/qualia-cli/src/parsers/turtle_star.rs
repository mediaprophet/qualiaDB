...
mod tests {
    ...
    fn test_virtual_id_context_independence() {
        use qualia_core_db::lexicon::TAG_EMBEDDED;
...
impl crate::rdf_star::RdfStarSerializer for CborLdStarSerializer {
    fn serialize_embedded_triple(
        ...
        let mut encoder = ser::into_writer(&mut buffer).map_err(|_| crate::rdf_star::RdfStarSerializeError::BufferTooSmall)?;
        
        // Write tag 103
        ciborium::tag!(103).serialize(&mut encoder).map_err(|_| crate::rdf_star::RdfStarSerializeError::BufferTooSmall)?;
        
        // Write array of 3 integers (component hashes)
        // TODO: This should write full IRIs/blank nodes, not just hashes
        // For now, write the hashes as integers
        components.serialize(&mut encoder).map_err(|_| crate::rdf_star::RdfStarSerializeError::BufferTooSmall)?;
    ...
    fn serialize_triple(
        ...
        [subject, predicate, object].serialize(&mut encoder).map_err(|_| crate::rdf_star::RdfStarSerializeError::BufferTooSmall)?;
    ...
    fn serialize_quad(
        ...
        [subject, predicate, object, graph].serialize(&mut encoder).map_err(|_| crate::rdf_star::RdfStarSerializeError::BufferTooSmall)?;
...
mod cbor_serializer_tests {
    ...
    fn test_serialize_embedded_triple() {
        ...
        // Should start with CBOR tag 103 (0x67)
        assert_eq!(bytes[0], 0x67); // Tag 103 in CBOR
    }
...
impl crate::rdf_star::RdfStarSerializer for N3StarSerializer {
    ...
    fn serialize_triple(
        ...
        // Try to use variable name if available
        let s = self.variables.get(&subject).map(|s| s.as_str()).unwrap_or(&format!("_:{}", subject));
        let p = self.variables.get(&predicate).map(|s| s.as_str()).unwrap_or(&format!("_:{}", predicate));
        let o = self.variables.get(&object).map(|s| s.as_str()).unwrap_or(&format!("_:{}", object));
