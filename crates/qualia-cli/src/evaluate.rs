use qualia_core_db::deontic_logic::{evaluate_accessible_layers, VcAttributes};
...
pub fn run_deontic(dataset: &Path, contract_hash: u64) {
    ...
    let vault = match qualia_core_db::key_vault::KeyVault::load_or_generate(&storage_dir) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("KeyVault error: {e}");
            return;
        }
    };
    let mut out: [Option<(
        qualia_core_db::key_vault::SubgraphLayer,
        qualia_core_db::key_vault::SubgraphKey,
...
pub fn run_ltl(dataset: &Path, formula_type: &str, hash_a: u64, hash_b: u64) {
    ...
    println!(
        "LTL trace evaluation ({formula_type} 0x{hash_a:x}): {}",
        if result { "HOLDS" } else { "VIOLATED" }
    );
}
...
pub fn run_linear_logic(dataset: &Path, quin_index: usize) {
    ...
    match quins.get(quin_index) {
        None => eprintln!(
            "Quin index {quin_index} out of range (dataset has {} quins).",
            quins.len()
        ),
        Some(q) => {
            ...
            println!(
                "  After  consume: {}",
                if is_consumed(&mutable_q) {
                    "consumed"
                } else {
                    "still available (check bit encoding)"
                }
            );
        }
    }
...
pub fn run_diffusion(graph_id: &str) {
    ...
    println!(
        "Diffusion trigger for graph '{}': {}",
        graph_id,
        if triggered {
            "initiated"
        } else {
            "already active or no-op"
        }
    );
}
...
pub fn run_spatio_temporal(
    ...
    match action.to_ascii_lowercase().as_str() {
        "rcc8" => {
            let region_a =
                SpatialRegion::new(1, vec![(ax1, ay1), (ax2, ay1), (ax2, ay2), (ax1, ay2)]);
            let region_b =
                SpatialRegion::new(2, vec![(bx1, by1), (bx2, by1), (bx2, by2), (bx1, by2)]);
            let relation = evaluate_rcc8(&region_a, &region_b);
            println!("RCC8 relation A({ax1},{ay1})-({ax2},{ay2}) vs B({bx1},{by1})-({bx2},{by2}): {relation:?}");
        }
        "temporal-before" => {
            let result = evaluate_temporal(
                TemporalOp::Before,
                ax1 as i64,
                ay1 as i64,
                bx1 as i64,
...
pub fn run_interval(action: &str, start1: i64, end1: i64, start2: i64, end2: i64, point: i64) {
    ...
    match action.to_ascii_lowercase().as_str() {
        "contains" => println!(
            "Interval [{start1}..{end1}].contains({point}): {}",
            a.contains(point)
        ),
        "overlaps" => println!(
            "Interval [{start1}..{end1}] overlaps [{start2}..{end2}]: {}",
            a.overlaps(&b)
        ),
        "intersection" => match a.intersection(&b) {
...
pub fn run_argumentation(demo: bool, dataset: Option<&Path>) {
    use qualia_core_db::modalities::argumentation::{
        create_sanctuary_debate, ArgumentationFramework,
    };
...
pub fn run_neuro_symbolic() {
    ...
    for i in 0..spec.subjects_len as usize {
        println!(
            "    Subject[{i}]   : 0x{:016x}  (q_hash(\"Patient\") = 0x{:016x})",
            spec.subjects[i],
            q_hash("Patient")
        );
    }
    for i in 0..spec.predicates_len as usize {
        println!(
            "    Predicate[{i}] : 0x{:016x}  (q_hash(\"fever\") = 0x{:016x})",
            spec.predicates[i],
            q_hash("fever")
        );
    }
    for i in 0..spec.objects_len as usize {
        println!(
            "    Object[{i}]    : 0x{:016x}  (q_hash(\"True\") = 0x{:016x})",
            spec.objects[i],
            q_hash("True")
        );
    }
    ...
    println!(
        "  graph_mutation_default spec ({} subjects, {} predicates, {} objects):",
