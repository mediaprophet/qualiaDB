...
pub enum SolveAction {
    Linalg { #[command(subcommand)] action: LinalgAction },
    Optimize { #[command(subcommand)] action: OptimizeAction },
    Ode { #[command(subcommand)] action: OdeAction },
    Quantum { #[command(subcommand)] action: QuantumSolveAction },
    Symbolic { #[command(subcommand)] action: SymbolicSolveAction },
}
...
pub enum LinalgAction {
    Multiply { #[arg(long)] matrix_a: String, #[arg(long)] matrix_b: String },
    Determinant { #[arg(long)] matrix: String },
    SolveSystem { #[arg(long)] matrix: String, #[arg(long)] vector: String },
    Eigenvalues { #[arg(long)] matrix: String, #[arg(long, default_value = "4")] count: usize },
    TensorContract { #[arg(long)] tensor: String },
}
