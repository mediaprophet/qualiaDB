...
mod tests {
    ...
    fn test_daemon_swarm_fiduciary_boundary_stress() {
        ...
        println!(
            "Fiduciary Boundary Stress Test Completed. Approvals: {}, Denials: {}",
            approved_count, denied_count
        );
        assert_eq!(
            denied_count + approved_count,
            iterations,
            "All traffic must be definitively routed or dropped"
        );
        // Classified is 1/3 of iterations (3333). Half of restricted (1/3) is ~1666.
        // Denials should be roughly 5000.
        assert!(
            denied_count >= 3000,
            "Gatekeeper failed to deny restricted/classified streams"
        );
    }
}
