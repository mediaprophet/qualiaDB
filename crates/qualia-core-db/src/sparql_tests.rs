...
mod sparql_tests {
    ...
    fn test_ast_creation() {
        ...
        let pattern_id = ctx.add_pattern(Pattern::Triple {
            subject: 0x1,
            predicate: 0x2,
            object: 0x3,
        });
    ...
    fn test_ast_pattern_limit() {
        let mut ctx = SparqlQueryContext::new();
        for i in 0..128 {
            ctx.add_pattern(Pattern::Triple {
                subject: i as u64,
    ...
    fn test_ast_pattern_overflow() {
        ...
        let result = ctx.add_pattern(Pattern::Triple {
            subject: 0x1,
            predicate: 0x2,
            object: 0x3,
        });
    ...
    fn test_ast_property_path() {
        let mut ctx = SparqlQueryContext::new();
        let path_id = ctx.add_path(Path::Sequence {
            first: 0x1,
            rest: 0x2,
        });
        assert_eq!(path_id, 0);
    ...
    fn test_ast_subquery() {
        let mut ctx = SparqlQueryContext::new();
        let subquery_id = ctx.add_subquery(Subquery {
            query_type: QueryType::Select,
            projection: [0; 16],
            projection_count: 1,
            where_clause: 0,
        });
        assert_eq!(subquery_id, 0);
    }

    #[test]
    fn test_ast_embedded_triple() {
        let ctx = SparqlQueryContext::new();
        let embedded = EmbeddedTriple {
            subject: 0x1,
            predicate: 0x2,
            object: 0x3,
        };
        assert_eq!(embedded.subject, 0x1);
    }

    #[test]
    fn test_ast_service_pattern() {
        let mut ctx = SparqlQueryContext::new();
        let pattern_id = ctx.add_pattern(Pattern::Service {
    ...
    fn test_planner_triple_scan() {
        ...
        let result = LogicalPlanner::plan_pattern(pattern_id, &ctx, &mut plan);
        assert!(result.is_ok());
    ...
    fn test_planner_filter() {
        ...
        let expr_id = ctx.add_expression(Expression::Literal(1)).unwrap();
        let filter_id = ctx.add_pattern(Pattern::Filter {
            pattern: pattern_id,
            expression: expr_id,
        }).unwrap();
        
        let mut plan = ExecutionPlan::new();
        let result = LogicalPlanner::plan_pattern(filter_id, &ctx, &mut plan);
        assert!(result.is_ok());
    }
    ...
    fn test_executor_triple_scan() {
        ...
        let executor = QueryExecutor { quins: &quins };
    ...
    fn test_filter_equality() {
        ...
        let ctx = SparqlQueryContext::new();
        let result = FilterEvaluator::evaluate(&expr, &ctx, &mut row);
        assert!(result.is_ok());
        assert!(result.unwrap() == 1); // true
    }
    ...
    fn test_filter_bound() {
        ...
        let expr = Expression::FunctionCall {
            func: FunctionType::Bound,
            args: [ExpressionId(0), 0, 0, 0],
            arg_count: 1,
        };
    ...
    fn test_full_query_pipeline() {
        ...
        let mut plan = ExecutionPlan::new();
        let planned = LogicalPlanner::plan_pattern(pattern_id, &ctx, &mut plan);
    ...
    fn test_zero_allocation_compliance() {
        ...
        for i in 0..128 {
            let _ = ctx.add_pattern(Pattern::Triple {
                subject: i as u64,
                predicate: i as u64 + 1,
                object: i as u64 + 2,
            });
    ...
    fn test_did_prefix_recognition() {
        ...
        let did_with_prefix = 0x8000000000000001;
        let did_without_prefix = 0x0000000000000001;
        
        assert_ne!(did_with_prefix & 0x8000000000000000, 0);
        assert_eq!(did_without_prefix & 0x8000000000000000, 0);
    }

    #[test]
    fn test_virtual_id_hash_strategy() {
        // Verify 0x1 prefix for Virtual ID Hash
        let virtual_id = 0x1000000000000001;
        
        assert_ne!(virtual_id & 0x1000000000000000, 0);
    }
}