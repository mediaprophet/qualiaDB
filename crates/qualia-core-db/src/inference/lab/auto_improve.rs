...
pub fn run_auto_improve(cfg: &AutoImproveConfig) -> Result<LockInPackage, String> {
    ...
    while generation < cfg.max_generations && t_start.elapsed() < cfg.max_duration {
        ...
        for cfg_point in queue.drain(..) {
            ...
            let mut run = ExperimentRun {
                ...
                backend: trial.config.backend.unwrap_or("auto").to_string(),
    ...
    let methodology =
        build_methodology(&best, ollama_tok_s, a_gap, &audit, &all_trials, generation);
...
mod tests {
    ...
    fn search_space_nonempty_and_dedup_labels() {
        ...
        assert!(
            space.len() >= 8,
            "expected a real search grid, got {}",
            space.len()
        );
        ...
        assert_eq!(
            labels.len(),
            space.len(),
            "duplicate labels in search space"
        );
    }
