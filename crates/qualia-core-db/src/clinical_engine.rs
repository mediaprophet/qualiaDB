...
pub fn framingham_10yr_risk(input: &FraminghamInput) -> FraminghamResult {
    ...
    let (b_age, b_tc, b_hdl, b_sbp_unt, b_sbp_trt, b_smoke, b_diab, baseline_surv, mean_sum) =
        if input.sex_male {
            (3.06117, 1.12370, -0.93263, 1.93303, 1.99881, 0.65451, 0.57367, 0.88936_f64, 23.9802_f64)
        } else {
            (2.32888, 1.20904, -0.70833, 2.76157, 2.82263, 0.52873, 0.69154, 0.95012_f64, 26.1931_f64)
        };

    let sbp_coeff = if input.bp_treated { b_sbp_trt } else { b_sbp_unt };
    ...
    let category = if risk_10yr < 0.10      { RiskCategory::Low }
        else if risk_10yr <= 0.20           { RiskCategory::Moderate }
        else                                { RiskCategory::High };

    FraminghamResult { risk_10yr, category, log_score }
}
...
pub fn cha2ds2_vasc_score(input: &Cha2ds2VascInput) -> Cha2ds2VascResult {
    ...
    let annual_risk = match score {
        ...
        5 => 6.7, 6 => 9.8, 7 => 9.6, 8 => 12.5, _ => 15.2,
    };

    let anticoagulation_recommended = if input.sex_female { score >= 3 } else { score >= 2 };

    Cha2ds2VascResult { score, annual_stroke_risk_pct: annual_risk, anticoagulation_recommended }
}
...
pub fn score2_risk(input: &Score2Input) -> Score2Result {
    ...
    let calibrated_pct = (base_risk * match input.risk_region {
    ...
    let category = if calibrated_pct < 5.0     { RiskCategory::Low }
        else if calibrated_pct <= 10.0         { RiskCategory::Moderate }
        else if calibrated_pct <= 20.0         { RiskCategory::High }
        else                                   { RiskCategory::VeryHigh };

    Score2Result { risk_10yr_pct: calibrated_pct, category }
}
...
pub fn check_drug_interactions(active_medications: &[u64]) -> Vec<DrugInteraction> {
    ...
    let pairs: &[(&str, &str, InteractionSeverity, &str)] = &[
        ("warfarin",     "ibuprofen",       InteractionSeverity::Major,           "CYP2C9 inhibition + antiplatelet effect → major bleeding"),
        ("warfarin",     "naproxen",        InteractionSeverity::Major,           "CYP2C9 inhibition + antiplatelet effect → major bleeding"),
        ("warfarin",     "aspirin",         InteractionSeverity::Moderate,        "Additive antiplatelet: monitor INR closely"),
        ("sertraline",   "phenelzine",      InteractionSeverity::Contraindicated, "Serotonin syndrome risk (SSRI + MAOI)"),
        ("fluoxetine",   "selegiline",      InteractionSeverity::Contraindicated, "Serotonin syndrome risk (SSRI + MAO-B)"),
        ("simvastatin",  "clarithromycin",  InteractionSeverity::Major,           "CYP3A4 inhibition → rhabdomyolysis risk"),
        ("atorvastatin", "clarithromycin",  InteractionSeverity::Moderate,        "CYP3A4 inhibition → myopathy risk"),
        ("amiodarone",   "ciprofloxacin",   InteractionSeverity::Major,           "Additive QT prolongation → TdP risk"),
        ("methadone",    "azithromycin",    InteractionSeverity::Major,           "Additive QT prolongation → TdP risk"),
        ("lisinopril",   "spironolactone",  InteractionSeverity::Moderate,        "Hyperkalaemia risk (ACEi + K-sparing diuretic)"),
        ("ramipril",     "spironolactone",  InteractionSeverity::Moderate,        "Hyperkalaemia risk (ACEi + K-sparing diuretic)"),
        ("metformin",    "iohexol",         InteractionSeverity::Major,           "Lactic acidosis risk — hold metformin 48h before iodinated contrast"),
        ("lithium",      "ibuprofen",       InteractionSeverity::Major,           "NSAIDs reduce renal lithium clearance → toxicity"),
        ("lithium",      "diclofenac",      InteractionSeverity::Major,           "NSAIDs reduce renal lithium clearance → toxicity"),
        ("methotrexate", "trimethoprim",    InteractionSeverity::Major,           "Additive folate antagonism → pancytopenia"),
        ("digoxin",      "amiodarone",      InteractionSeverity::Major,           "Amiodarone increases digoxin levels → toxicity"),
        ("clopidogrel",  "omeprazole",      InteractionSeverity::Moderate,        "CYP2C19 inhibition reduces clopidogrel activation"),
        ("tramadol",     "sertraline",      InteractionSeverity::Moderate,        "Serotonin syndrome + seizure threshold lowering"),
    ];
        ...
        for &b in &active_medications[i + 1..] {
            for &(na, nb, sev, mech) in pairs {
                ...
                if (a == ha && b == hb) || (a == hb && b == ha) {
                    found.push(DrugInteraction { drug_a: a, drug_b: b, severity: sev, mechanism: mech });
                }
            }
        }
    }
...
pub fn check_contraindications(drug_hash: u64, condition_hashes: &[u64]) -> Vec<ContraindicationResult> {
    // (drug_name, snomed_name_as_hashed, severity, reason)
    let table: &[(&str, &str, InteractionSeverity, &str)] = &[
        ("metformin",  "709044004",  InteractionSeverity::Contraindicated, "CKD stage 4/5 (eGFR < 30): lactic acidosis risk"),
        ("nsaid",      "709044004",  InteractionSeverity::Major,           "CKD: NSAIDs worsen renal haemodynamics"),
        ("ibuprofen",  "709044004",  InteractionSeverity::Major,           "CKD: NSAIDs worsen renal haemodynamics"),
        ("naproxen",   "709044004",  InteractionSeverity::Major,           "CKD: NSAIDs worsen renal haemodynamics"),
        ("atenolol",   "195967001",  InteractionSeverity::Contraindicated, "Asthma: non-selective beta-blockers precipitate bronchospasm"),
        ("propranolol","195967001",  InteractionSeverity::Contraindicated, "Asthma: non-selective beta-blockers precipitate bronchospasm"),
        ("metoprolol", "195967001",  InteractionSeverity::Moderate,        "Asthma: cardioselective beta-blocker — use with caution"),
        ("lithium",    "709044004",  InteractionSeverity::Contraindicated, "CKD: lithium is renally cleared — nephrotoxicity risk"),
        ("clozapine",  "84989004",   InteractionSeverity::Major,           "Seizure disorder: clozapine lowers seizure threshold"),
        ("tramadol",   "84989004",   InteractionSeverity::Major,           "Seizure disorder: tramadol lowers seizure threshold"),
        ("warfarin",   "713078009",  InteractionSeverity::Major,           "Haemorrhagic stroke history: anticoagulation risk"),
        ("amiodarone", "49436004",   InteractionSeverity::Major,           "Pulmonary disease: amiodarone pulmonary toxicity risk"),
        ("thalidomide","77386006",   InteractionSeverity::Contraindicated, "Pregnancy: teratogen (Category X)"),
        ("isotretinoin","77386006",  InteractionSeverity::Contraindicated, "Pregnancy: teratogen (Category X)"),
        ("methotrexate","77386006",  InteractionSeverity::Contraindicated, "Pregnancy: teratogen — folate antagonist"),
        ("sildenafil", "194828000",  InteractionSeverity::Contraindicated, "Angina on nitrates: severe hypotension risk"),
    ];

    condition_hashes.iter().filter_map(|&cond| {
        table.iter().find(|&&(dn, sn, _, _)| crate::q_hash(dn) == drug_hash && crate::q_hash(sn) == cond)
...
pub fn validate_fhir_observation(obs: &FhirObservation) -> FhirValidationResult {
    ...
    let range = RANGES.iter().find(|(code, ..)| *code == obs.loinc_code.as_str());

    let status = if let Some(&(_, low, high, crit_low, crit_high)) = range {
        let v = obs.value;
        if v < crit_low      { ObservationStatus::CriticalLow }
        else if v > crit_high { ObservationStatus::CriticalHigh }
        else if v < low       { ObservationStatus::Low }
        else if v > high      { ObservationStatus::High }
        else                  { ObservationStatus::Normal }
    } else if let (Some(low), Some(high)) = (obs.reference_low, obs.reference_high) {
        let v = obs.value;
        if v < low * 0.5      { ObservationStatus::CriticalLow }
        else if v > high * 2.0 { ObservationStatus::CriticalHigh }
        else if v < low        { ObservationStatus::Low }
        else if v > high       { ObservationStatus::High }
        else                   { ObservationStatus::Normal }
    } else {
    ...
    FhirValidationResult { is_valid: status != ObservationStatus::Unknown, status, interpretation_code: interp }
}
...
pub fn longitudinal_trend(series: &[TimePoint], forecast_days: f64, improvement_direction: i8) -> TrendResult {
    if series.len() < 2 {
        let v = series.first().map(|p| p.value).unwrap_or(0.0);
        return TrendResult { slope_per_day: 0.0, r_squared: 0.0, forecast: v, direction: TrendDirection::Insufficient };
    }

    let t0 = series[0].timestamp_s as f64;
    let xs: Vec<f64> = series.iter().map(|p| (p.timestamp_s as f64 - t0) / 86400.0).collect();
    ...
    if denom.abs() < 1e-10 {
        return TrendResult { slope_per_day: 0.0, r_squared: 1.0, forecast: ys[0], direction: TrendDirection::Stable };
    }
    ...
    let ss_res: f64 = ys.iter().zip(xs.iter()).map(|(y, x)| (y - (intercept + slope * x)).powi(2)).sum();
    let ss_tot: f64 = ys.iter().map(|y| (y - y_mean).powi(2)).sum();
    let r_squared = if ss_tot < 1e-10 { 1.0 } else { 1.0 - ss_res / ss_tot };
    ...
    } else if (slope > 0.0 && improvement_direction > 0) || (slope < 0.0 && improvement_direction < 0) {
        TrendDirection::Improving
    } else {
        TrendDirection::Worsening
    };

    TrendResult { slope_per_day: slope, r_squared, forecast, direction }
}
...
pub fn evaluate_gene_expression(gene_id: u64, baseline: f64, treatment: f64, fc_threshold: f64) -> GeneExpressionResult {
    let fold_change = if baseline > 1e-9 { treatment / baseline } else { f64::INFINITY };
    let log2_fc = if fold_change.is_finite() { fold_change.log2() } else { f64::INFINITY };
    let is_significant = fold_change >= fc_threshold || (fold_change.is_finite() && fold_change <= 1.0 / fc_threshold);

    let direction = if !is_significant { ExpressionDirection::Unchanged }
        else if fold_change >= 1.0    { ExpressionDirection::Upregulated }
        else                          { ExpressionDirection::Downregulated };
...
mod tests {
    ...
    fn framingham_high_risk_male() {
        let r = framingham_10yr_risk(&FraminghamInput {
            age: 60, sex_male: true,
            total_cholesterol_mmol: 6.5, hdl_cholesterol_mmol: 0.9,
            systolic_bp: 162.0, bp_treated: false,
    ...
    fn framingham_low_risk_female() {
        let r = framingham_10yr_risk(&FraminghamInput {
            age: 40, sex_male: false,
            total_cholesterol_mmol: 4.5, hdl_cholesterol_mmol: 1.8,
    ...
    fn cha2ds2_max_score() {
        let r = cha2ds2_vasc_score(&Cha2ds2VascInput {
            ...
            age_75_or_older: true, diabetes: true,
            stroke_tia_history: true, vascular_disease: true,
            age_65_to_74: false, sex_female: true,
        });
        assert_eq!(r.score, 9);
    ...
    fn fhir_hba1c_normal() {
        let r = validate_fhir_observation(&FhirObservation {
            loinc_code: "4548-4".into(), value: 5.2,
            unit_ucum: "%".into(), reference_low: None, reference_high: None,
        });
    ...
    fn fhir_hba1c_high() {
        let r = validate_fhir_observation(&FhirObservation {
            loinc_code: "4548-4".into(), value: 8.5,
            unit_ucum: "%".into(), reference_low: None, reference_high: None,
        });
    ...
    fn trend_worsening_bp() {
        let series = vec![
            TimePoint { timestamp_s: 0,       value: 120.0 },
            TimePoint { timestamp_s: 86400,   value: 125.0 },
            TimePoint { timestamp_s: 172800,  value: 130.0 },
            TimePoint { timestamp_s: 259200,  value: 135.0 },
        ];
