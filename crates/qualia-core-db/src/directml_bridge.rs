...
impl DmlDevice {
    ...
    pub fn new() -> windows::core::Result<Self> {
        unsafe {
            ...
            let queue: ID3D12CommandQueue = d3d12.CreateCommandQueue(&D3D12_COMMAND_QUEUE_DESC {
            ...
            Ok(Self { d3d12, dml, queue, adapter_desc })
        }
    }
                ...
                Ok(a) => {
                    if let Ok(desc) = a.GetDesc1() {
                        ...
                        if desc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE.0 as u32 == 0 {
                            ...
                            if vram > best_vram { best_vram = vram; best = Some(a); }
                        }
                    }
                    idx += 1;
                }
...
unsafe fn commit_buffer(
    ...
    let heap = D3D12_HEAP_PROPERTIES { Type: heap_type, ..Default::default() };
    let mut out: Option<ID3D12Resource> = None;
    d3d.CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &desc, initial_state, None, &mut out)?;
    Ok(out.unwrap())
}

unsafe fn upload_buffer(d3d: &ID3D12Device, data: &[u8]) -> windows::core::Result<ID3D12Resource> {
    let buf = commit_buffer(d3d, data.len() as u64,
...
unsafe fn fence_wait(d3d: &ID3D12Device, queue: &ID3D12CommandQueue, value: u64)
    -> windows::core::Result<()>
{
...
pub struct DmlGemmOp { pub m: u32, pub k: u32, pub n: u32 }
...
impl DmlGemmOp {
    ...
    pub fn execute(
        ...
        unsafe {
            ...
            device.dml.CompileOperator(&op, DML_EXECUTION_FLAGS(0), &mut compiled_opt)?;
            ...
            let heap: ID3D12DescriptorHeap = d3d.CreateDescriptorHeap(&D3D12_DESCRIPTOR_HEAP_DESC {
            ...
            // ── Initialise ────────────────────────────────────────────────────
            let init_op: IDMLOperatorInitializer =
                device.dml.CreateOperatorInitializer(Some(&[Some(compiled.clone())]))?;
            ...
            let bb_a = DML_BUFFER_BINDING {
                ...
                Offset: 0, SizeInBytes: (m * k * 4) as u64,
            };
            let bb_b = DML_BUFFER_BINDING {
                Buffer: ManuallyDrop::new(Some(buf_b.clone())),
                Offset: 0, SizeInBytes: (k * n * 4) as u64,
            };
            let bb_c = DML_BUFFER_BINDING {
                Buffer: ManuallyDrop::new(Some(buf_c.clone())),
                Offset: 0, SizeInBytes: out_bytes,
            };

            let in_binds = [
                DML_BINDING_DESC { Type: DML_BINDING_TYPE_BUFFER, Desc: &bb_a as *const _ as _ },
                DML_BINDING_DESC { Type: DML_BINDING_TYPE_BUFFER, Desc: &bb_b as *const _ as _ },
            ];
            let out_bind = DML_BINDING_DESC { Type: DML_BINDING_TYPE_BUFFER, Desc: &bb_c as *const _ as _ };
...
fn make_tensor_desc(sizes: &[u32; 2], byte_size: u64) -> DML_TENSOR_DESC {
    ...
    DML_TENSOR_DESC { Type: DML_TENSOR_TYPE_BUFFER, Desc: inner as *const _ }
}
