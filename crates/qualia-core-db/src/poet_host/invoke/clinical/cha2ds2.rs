...
pub fn score(args_v: &Value, _span: Span) -> Result<Value, Diagnostic> {
    ...
    let input = Cha2ds2VascInput {
        congestive_heart_failure: args::rec_bool(args_v, "congestive_heart_failure").unwrap_or(false),
        ...
        age_65_to_74: (65..75).contains(&age) || args::rec_bool(args_v, "age_65_to_74").unwrap_or(false),
        sex_female: args::rec_bool(args_v, "sex_female").unwrap_or(false),
    };
    let r = cha2ds2_vasc_score(&input);
    Ok(args::record([
        ("score", Value::U64(r.score as u64)),
        ("annual_stroke_risk_percent", Value::F64(r.annual_stroke_risk_pct)),
        ("anticoagulation_indicated", Value::Bool(r.anticoagulation_recommended)),
    ]))
}
