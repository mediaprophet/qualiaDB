...
use crate::solvers::vector_calculus::integrals::{
    line_integral_scalar, line_integral_work, surface_flux,
};
...
fn eval_expr(expr: &Expr, env: &HashMap<String, f64>, span: Span) -> Result<f64, Diagnostic> {
    expr.eval(env).ok_or_else(|| {
        args::bad(
            span,
            "evaluation failed (unbound variable or non-finite result)",
        )
    })
}
...
pub fn ode_rk4_integrate(args_v: &Value, span: Span) -> Result<Value, Diagnostic> {
    ...
    if t_span.len() != 2 || t_span[1] <= t_span[0] {
        return Err(args::bad(
            span,
            "t_span must be [t0, t_final] with t_final > t0",
        ));
    }
    let t0 = t_span[0];
    ...
    let system_strs = if let Some(sys) = args::rec_str_list(args_v, "system") {
        ...
        return Err(args::bad(
            span,
            "Ode.rk4_integrate needs system: [string] or string",
        ));
    };
    ...
    if system_strs.len() != n {
        return Err(args::bad(
            span,
            "number of system equations must match number of variables",
        ));
    }
    ...
    let y0_vec = if let Some(y_list) = args::rec_f64_list(args_v, "y0") {
        ...
        return Err(args::bad(
            span,
            "Ode.rk4_integrate needs y0: [number] or number",
        ));
    };
    if y0_vec.len() != n {
        return Err(args::bad(
            span,
            "initial state y0 dimension must match number of variables",
        ));
    }
    ...
    let mut trajectory = vec![Value::List(
        current_y.iter().map(|&v| Value::F64(v)).collect(),
    )];
    ...
    if num_steps > max_steps {
        return Err(args::bad(
            span,
            format!("step count {num_steps} exceeds maximum of {max_steps}"),
        ));
    }
    ...
    for _ in 0..num_steps {
        ...
        for i in 0..n {
            y_temp[i] = current_y[i] + 0.5 * h * k1[i];
        }
        let k2 = eval_deriv(current_t + 0.5 * h, &y_temp)?;

        for i in 0..n {
            y_temp[i] = current_y[i] + 0.5 * h * k2[i];
        }
        let k3 = eval_deriv(current_t + 0.5 * h, &y_temp)?;

        for i in 0..n {
            y_temp[i] = current_y[i] + h * k3[i];
        }
...
pub fn ode_dopri5(args_v: &Value, span: Span) -> Result<Value, Diagnostic> {
    ...
    if system_strs.len() != n {
        return Err(args::bad(
            span,
            "dimension mismatch between vars and system",
        ));
    }
        ...
        Ok(res) => {
            ...
            Ok(args::record([
                ...
                (
                    ...
                    Value::I64(res.derivative_evaluations as i64),
                ),
...
pub fn ode_bdf(args_v: &Value, span: Span) -> Result<Value, Diagnostic> {
    ...
    let dt =
        args::rec_f64(args_v, "dt").ok_or_else(|| args::bad(span, "Ode.bdf needs dt: number"))?;
    if dt <= 0.0 {
...
pub fn ode_symplectic_step(args_v: &Value, span: Span) -> Result<Value, Diagnostic> {
    let force_str = args::rec_str(args_v, "force").ok_or_else(|| {
        args::bad(
            span,
            "Ode.symplectic_step needs force: string (e.g. '-1.0 * q')",
        )
    })?;
    ...
    for i in 0..n {
        ...
        for j in 0..n {
            let h_ij = if i == j {
                ...
                p_mm[i] -= h;
                p_mm[j] -= h;
                (eval_at(&p_pp)? - eval_at(&p_pm)? - eval_at(&p_mp)? + eval_at(&p_mm)?)
                    / (4.0 * h * h)
            };
...
pub fn calc_newton_solve(args_v: &Value, span: Span) -> Result<Value, Diagnostic> {
    ...
    if expr_strs.len() != n || guess.len() != n {
        return Err(args::bad(
            span,
            "dimension mismatch: exprs, vars, guess must have equal length",
        ));
    }
    ...
    for iter in 0..max_iter {
        ...
        for col in 0..n {
            ...
            let mut env_plus = HashMap::new();
            for (i, v) in vars.iter().enumerate() {
                env_plus.insert(v.clone(), pt_plus[i]);
            }
            for row in 0..n {
        ...
        if let Some(delta) = solve_linear_system(&j_mat, &fx, n) {
            ...
            return Err(args::bad(
                span,
                "singular Jacobian encountered during Newton step",
            ));
        }
    }
...
pub fn opt_simulated_annealing(args_v: &Value, span: Span) -> Result<Value, Diagnostic> {
    let obj_str = args::rec_str(args_v, "objective").ok_or_else(|| {
        args::bad(
            span,
            "Optimization.simulated_annealing needs objective: string",
        )
    })?;
    let vars = args::rec_str_list(args_v, "vars").ok_or_else(|| {
        args::bad(
            span,
            "Optimization.simulated_annealing needs vars: [string]",
        )
    })?;
    let raw_bounds = args::rec(args_v, "bounds")
        ...
        .ok_or_else(|| {
            args::bad(
                span,
                "Optimization.simulated_annealing needs bounds: [[min, max]]",
            )
        })?;
    ...
    for b in raw_bounds {
        ...
        if pair.len() != 2 {
            return Err(args::bad(span, "each bound must be [min, max]"));
        }
        let min =
            args::as_f64(&pair[0]).ok_or_else(|| args::bad(span, "bound min must be number"))?;
        let max =
            args::as_f64(&pair[1]).ok_or_else(|| args::bad(span, "bound max must be number"))?;
        bounds.push((min, max));
    }
    ...
    let mut current = if let Some(init) = args::rec_f64_list(args_v, "initial") {
        if init.len() != n {
            return Err(args::bad(span, "initial state length must match vars"));
        }
        init
    } else {
        bounds
            .iter()
            .map(|(min, max)| min + rng.unit() * (max - min))
            .collect()
    };
...
pub fn opt_artificial_bee_colony(args_v: &Value, span: Span) -> Result<Value, Diagnostic> {
    let obj_str = args::rec_str(args_v, "objective").ok_or_else(|| {
        args::bad(
            span,
            "Optimization.artificial_bee_colony needs objective: string",
        )
    })?;
    let vars = args::rec_str_list(args_v, "vars").ok_or_else(|| {
        args::bad(
            span,
            "Optimization.artificial_bee_colony needs vars: [string]",
        )
    })?;
    let raw_bounds = args::rec(args_v, "bounds")
        .and_then(args::list)
        .ok_or_else(|| {
            args::bad(
                span,
                "Optimization.artificial_bee_colony needs bounds: [[min, max]]",
            )
        })?;
    ...
    for _ in 0..max_cycles {
        ...
        for i in 0..food_sources {
            ...
            while partner == i && food_sources > 1 {
                partner = rng.below(food_sources);
            }
...
pub fn vc_line_integral_scalar(args_v: &Value, span: Span) -> Result<Value, Diagnostic> {
    let expr_str = args::rec_str(args_v, "expr").ok_or_else(|| {
        args::bad(
            span,
            "VectorCalculus.line_integral_scalar needs expr: string",
        )
    })?;
    let curve_strs = args::rec_str_list(args_v, "curve").ok_or_else(|| {
        args::bad(
            span,
            "VectorCalculus.line_integral_scalar needs curve: [string]",
        )
    })?;
    let t_span = args::rec_f64_list(args_v, "t_span").ok_or_else(|| {
        args::bad(
            span,
            "VectorCalculus.line_integral_scalar needs t_span: [t0, t1]",
        )
    })?;
    if t_span.len() != 2 {
        return Err(args::bad(span, "t_span must be [t0, t1]"));
    }
    ...
    let f = |pt: &[f64]| -> f64 {
        ...
        for (i, &coord) in pt.iter().enumerate() {
            if i < var_names.len() {
                env.insert(var_names[i].to_string(), coord);
            }
        }
    ...
    let curve = |t: f64| -> Vec<f64> {
        ...
        c_exprs
            .iter()
            .map(|e| e.eval(&env).unwrap_or(0.0))
            .collect()
    };
...
pub fn vc_line_integral_work(args_v: &Value, span: Span) -> Result<Value, Diagnostic> {
    let field_strs = args::rec_str_list(args_v, "field").ok_or_else(|| {
        args::bad(
            span,
            "VectorCalculus.line_integral_work needs field: [string]",
        )
    })?;
    let curve_strs = args::rec_str_list(args_v, "curve").ok_or_else(|| {
        args::bad(
            span,
            "VectorCalculus.line_integral_work needs curve: [string]",
        )
    })?;
    let t_span = args::rec_f64_list(args_v, "t_span").ok_or_else(|| {
        args::bad(
            span,
            "VectorCalculus.line_integral_work needs t_span: [t0, t1]",
        )
    })?;
    if t_span.len() != 2 {
        return Err(args::bad(span, "t_span must be [t0, t1]"));
...
pub fn vc_surface_flux(args_v: &Value, span: Span) -> Result<Value, Diagnostic> {
    let field_strs = args::rec_str_list(args_v, "field").ok_or_else(|| {
        args::bad(
            span,
            "VectorCalculus.surface_flux needs field: [string] (3 components)",
        )
    })?;
    let surf_strs = args::rec_str_list(args_v, "surface").ok_or_else(|| {
        args::bad(
            span,
            "VectorCalculus.surface_flux needs surface: [string] (3 components)",
        )
    })?;
    if field_strs.len() != 3 || surf_strs.len() != 3 {
        return Err(args::bad(
            span,
            "field and surface must each have 3 components",
        ));
    }
    ...
    let field = |pt: &[f64]| -> Vec<f64> {
        let mut env = HashMap::new();
        if !pt.is_empty() {
            env.insert("x".to_string(), pt[0]);
        }
        if pt.len() > 1 {
            env.insert("y".to_string(), pt[1]);
        }
        if pt.len() > 2 {
            env.insert("z".to_string(), pt[2]);
        }
        vec![
    ...
    let val = surface_flux(
        field, surf, u_span[0], u_span[1], v_span[0], v_span[1], steps,
    );
    Ok(args::record([
        ("flux", Value::F64(val)),
        ("steps", Value::I64(steps as i64)),
...
pub fn xform_laplace_numeric(args_v: &Value, span: Span) -> Result<Value, Diagnostic> {
    let expr_str = args::rec_str(args_v, "expr").ok_or_else(|| {
        args::bad(
            span,
            "IntegralTransforms.laplace_numeric needs expr: string",
        )
    })?;
    let s = args::rec_f64(args_v, "s").ok_or_else(|| {
        args::bad(
            span,
            "IntegralTransforms.laplace_numeric needs s: number (s > 0)",
        )
    })?;
    ...
    match laplace_numeric(f, s, t_max, steps_even) {
        ...
        Err(e) => Err(args::bad(
            span,
            format!("numerical Laplace transform failed: {e:?}"),
        )),
    }
}
...
pub fn xform_laplace_symbolic(args_v: &Value, span: Span) -> Result<Value, Diagnostic> {
    let expr_str = args::rec_str(args_v, "expr").ok_or_else(|| {
        args::bad(
            span,
            "IntegralTransforms.laplace_symbolic needs expr: string",
        )
    })?;
    let expr = parse_expr(expr_str, span)?;
    ...
    match laplace_table(&expr) {
        ...
        Err(e) => Err(args::bad(
            span,
            format!("symbolic Laplace transform failed: {e:?}"),
        )),
    }
}
...
mod tests {
    ...
    fn rk4_exponential_decay() {
        ...
        m.insert(
            "system".into(),
            Value::List(vec![Value::String("-0.5 * y".into())]),
        );
        m.insert("vars".into(), Value::List(vec![Value::String("y".into())]));
        m.insert(
            "t_span".into(),
            Value::List(vec![Value::F64(0.0), Value::F64(2.0)]),
        );
    ...
    fn dopri5_adaptive_integration() {
        ...
        m.insert(
            "system".into(),
            Value::List(vec![Value::String("-1.0 * y".into())]),
        );
        m.insert("vars".into(), Value::List(vec![Value::String("y".into())]));
        m.insert(
            "t_span".into(),
    ...
    fn newton_root_finder() {
        ...
        m.insert(
            "exprs".into(),
            Value::List(vec![Value::String("x^2 - 4".into())]),
        );
    ...
    fn simulated_annealing_minimizer() {
        ...
        m.insert(
            "bounds".into(),
            Value::List(vec![Value::List(vec![Value::F64(0.0), Value::F64(10.0)])]),
        );
    ...
    fn vector_calculus_line_integral() {
        ...
        m.insert(
            "curve".into(),
            Value::List(vec![
                Value::String("cos(t)".into()),
                Value::String("sin(t)".into()),
            ]),
        );
        m.insert(
            "t_span".into(),
            Value::List(vec![Value::F64(0.0), Value::F64(std::f64::consts::PI)]),
        );
