...
fn has_common_knowledge(ck_buf: &[u64], claim_fingerprint: u64) -> bool {
    let mut i = 0;
    while i < ck_buf.len() {
        if ck_buf[i] == claim_fingerprint { return true; }
        i += 1;
    }
    false
}
...
pub fn evaluate_epistemic_frame(
    ...
    for &q in quins {
        let opcode = (q.predicate & 0xFF) as u8;
        if !matches!(opcode, OP_KNOWS | OP_BELIEVES | OP_COMMON_KNOWLEDGE) { continue; }

        if agent_did_hash != 0 && q.subject != agent_did_hash && opcode != OP_COMMON_KNOWLEDGE { continue; }
        if world_hash != 0 && q.context != world_hash { continue; }

        let mut certainty = ((q.predicate >> CERTAINTY_BIT_SHIFT) & 0xFF) as u8;
        if has_common_knowledge(active_ck, q.object) { certainty = 255; }
        ...
        if verdict_count >= out.len() { return Err(EpistemicError::OutputBufferFull); }

        out[verdict_count] = EpistemicVerdict { claim: q, status, certainty };
        verdict_count += 1;
    }
...
mod tests {
    ...
    fn build_epistemic_quin(subject: u64, opcode: u8, certainty: u8, claim_fingerprint: u64, world: u64) -> QualiaQuin {
        let predicate = (opcode as u64) | ((certainty as u64) << CERTAINTY_BIT_SHIFT);
        QualiaQuin { subject, predicate, object: claim_fingerprint, context: world, metadata: 0, parity: subject ^ predicate ^ claim_fingerprint ^ world }
    }

    #[test]
    fn test_single_agent_knows_claim_active() {
        let agent = q_hash("agent1"); let claim = q_hash("claim1");
        let q = build_epistemic_quin(agent, OP_KNOWS, 200, claim, 0);
        let mut out = [EpistemicVerdict { claim: q, status: EpistemicStatus::Skipped, certainty: 0 }; 4];
        assert_eq!(evaluate_epistemic_frame(&[q], agent, 0, &mut out).unwrap(), 1);
    ...
    fn test_common_knowledge_propagation() {
        let agent1 = q_hash("agent1"); let claim = q_hash("claim1");
        let ck = build_epistemic_quin(0, OP_COMMON_KNOWLEDGE, 255, claim, 0);
        let b = build_epistemic_quin(agent1, OP_BELIEVES, 50, claim, 0);
        let mut out = [EpistemicVerdict { claim: ck, status: EpistemicStatus::Skipped, certainty: 0 }; 4];
        assert_eq!(evaluate_epistemic_frame(&[ck, b], 0, 0, &mut out).unwrap(), 2);
    ...
    fn test_agent_filter_world_hash_mismatch() {
        let agent = q_hash("agent1"); let world = q_hash("world1"); let claim = q_hash("claim1");
        let q = build_epistemic_quin(agent, OP_KNOWS, 200, claim, world);
        let mut out = [EpistemicVerdict { claim: q, status: EpistemicStatus::Skipped, certainty: 0 }; 4];
        assert_eq!(evaluate_epistemic_frame(&[q], 0, q_hash("world2"), &mut out).unwrap(), 0);
    }

    #[test]
    fn test_empty_slice_zero_verdicts() {
        let mut out = [EpistemicVerdict { claim: build_epistemic_quin(0, 0, 0, 0, 0), status: EpistemicStatus::Skipped, certainty: 0 }; 4];
