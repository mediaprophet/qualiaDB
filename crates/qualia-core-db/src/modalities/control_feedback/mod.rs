...
pub mod advanced;
pub use advanced::{
    adaptive_gains, mimo_output, mimo_step, mit_rule_adapt, mpc_control, scheduled_gain,
};

use super::epistemic_boundaries::{
    degrade_claim_to_socratic, detect_referral_by_severity, detect_referral_trigger,
    identify_degradation_vector, DegradationVector, ReferralTrigger, SocraticDegradation,
};
...
impl PowerSystemController {
    ...
    pub fn update(&mut self, measurements: PowerSystemState) -> Vec<ControlAction> {
        ...
        let voltage_output = self
            .battery_voltage_controller
            .update(measurements.battery_voltage);
        ...
        let solar_output = self
            .solar_current_controller
            .update(measurements.solar_current);
