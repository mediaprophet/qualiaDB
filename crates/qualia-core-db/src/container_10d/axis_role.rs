...
mod tests {
    ...
    fn proposed_table_matches_option_a_confirmation() {
        ...
        for &i in &SELECTOR_AXES {
            assert_eq!(PROPOSED_AXIS_ROLES[i], AxisRole::Selector, "axis {} should be Selector", AXIS_ORDER[i]);
        }
        // x,y,z,t,α,σ are pure coordinates
        for &i in &[3usize, 4, 5, 6, 7, 9] {
            assert_eq!(PROPOSED_AXIS_ROLES[i], AxisRole::Coordinate, "axis {} should be Coordinate", AXIS_ORDER[i]);
        }
    ...
    fn coordinate_axes_cover_seven_lanes_including_mu() {
        ...
        assert!(COORDINATE_AXES.contains(&MU_AXIS), "mu is a coordinate under Option A");
    }
}
