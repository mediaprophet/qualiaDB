...
//
// This crate implements the AgentRuntime trait and the Sentinel-gated message
// protocol that governs every LLM interaction under the Principal-Agent Duty of Care.
//
// Architecture:
//   Principal (Natural Person)
//     └── qualiaDB Sentinel VM  ← GATEKEEPER (validates all I/O)
...
//   - Sentinel validates I/O before touching the semantic graph
...
/// Leaves the remaining 384MB for the Sentinel VM, SLG Arena, and WASM stack.
...
// ─── AgentIntent ─────────────────────────────────────────────────────────────
/// Structured intent message from LLM → Sentinel. Every call must declare
/// what it intends to do — the Sentinel validates this against the Rights Ontology
...
pub enum SentinelVerdict {
    ...
    /// The output was modified (sanitised) by the Sentinel before passing through.
    Sanitised { original_hash: u64 },
}

// ─── AgentOutput ─────────────────────────────────────────────────────────────
/// The raw output from the LLM, before Sentinel validation.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AgentOutput {
    /// The text generated by the LLM.
    pub text: String,
    /// Provenance citations — hashes of QualiuQuins this output is grounded in.
    /// MUST be non-empty: uncited outputs are blocked by the Sentinel.
...
pub trait AgentRuntime: Send + Sync {
    ...
    /// Submits an intent to the Sentinel for pre-flight validation.
    /// This MUST be called before `infer`. Callers must not proceed if
    /// the verdict is `Deny`.
    fn validate_intent(&self, intent: &AgentIntent) -> SentinelVerdict;

    /// Runs inference on the given prompt and graph context.
    /// `graph_context` is a serialised sub-graph slice provided by the Sentinel.
    fn infer(&self, prompt: &str, graph_context: &str) -> Result<AgentOutput, AgentError>;

    /// Submits the LLM output to the Sentinel for post-flight grounding check.
    /// The Sentinel verifies provenance citations exist in the live graph.
    fn validate_output(&self, output: &AgentOutput) -> SentinelVerdict;
...
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub enum AgentError {
    /// Sentinel blocked the intent before inference started.
    SentinelDenied { rule_violated: u64, reason: String },
...
impl AgentRuntime for LocalLlmAgent {
    ...
    fn validate_intent(&self, intent: &AgentIntent) -> SentinelVerdict {
        ...
        if intent.requested_graph_scope.iter().any(|&h| h == SANCTUARY_SCOPE_SENTINEL) {
            return SentinelVerdict::Deny {
...
/// Special sentinel hash marking a Sanctuary-flagged graph scope.
pub const SANCTUARY_SCOPE_SENTINEL: u64 = 0xDEAD_BABE_CAFE_0042;
...
mod tests {
    ...
    #[test]
    fn test_sentinel_blocks_outbound_network() {
        ...
        assert!(matches!(verdict, SentinelVerdict::Deny { .. }), "Sentinel must block outbound calls from local backend");
    }

    #[test]
    fn test_sentinel_blocks_sanctuary_scope() {
        let agent = make_agent();
        let intent = AgentIntent {
            intent_predicate: 0xAABB,
        ...
        let verdict = agent.validate_intent(&intent);
        assert!(matches!(verdict, SentinelVerdict::Deny { .. }), "Sentinel must block Sanctuary scope access");
    }

    #[test]
    fn test_sentinel_permits_valid_local_intent() {
        let agent = make_agent();
        let intent = AgentIntent {
            intent_predicate: 0xAABB,
        ...
        assert_eq!(agent.validate_intent(&intent), SentinelVerdict::Permit);
    }
    ...
    fn test_full_roundtrip_grounded_output() {
        ...
        assert_eq!(post_verdict, SentinelVerdict::Permit, "Grounded output should pass post-flight check");
    }

    #[test]
    fn test_sentinel_blocks_ungrounded_output() {
        let agent = make_agent();
        ...
        assert!(matches!(verdict, SentinelVerdict::Deny { .. }), "Sentinel must block ungrounded output");
    }
}
