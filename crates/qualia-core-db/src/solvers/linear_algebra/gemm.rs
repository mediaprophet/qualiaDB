...
pub fn gemm(
    ...
    for i in 0..m {
        for j in 0..n {
            ...
            c[idx] = if beta == 0.0 { alpha * s } else { alpha * s + beta * c[idx] };
        }
    }
    Ok(())
}
...
mod tests {
    ...
    fn gemm_alpha_beta_accumulate() {
        ...
        gemm(Transpose::No, Transpose::No, 2, 2, 2, 2.0, &a, &b, 3.0, &mut c).unwrap();
    ...
    fn gemm_beta_zero_ignores_garbage() {
        ...
        gemm(Transpose::No, Transpose::No, 2, 2, 2, 1.0, &a, &b, 0.0, &mut c).unwrap();
        approx(&c, &a);
    }
    ...
    fn gemm_transpose_a_normal_equations() {
        ...
        let mut c = [0.0; 4];
        gemm(Transpose::Yes, Transpose::No, 2, 2, 3, 1.0, &a, &a, 0.0, &mut c).unwrap();
    ...
    fn gemm_transpose_b() {
        ...
        let mut c = [0.0; 4];
        gemm(Transpose::No, Transpose::Yes, 2, 2, 3, 1.0, &a, &b, 0.0, &mut c).unwrap();
    ...
    fn gemm_via_transpose_equals_matvec() {
        ...
        gemm(Transpose::No, Transpose::No, 2, 1, 3, 1.0, &a, &x, 0.0, &mut y_gemm).unwrap();
        approx(&y_mv, &y_gemm);
    }
