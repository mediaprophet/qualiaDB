...
//!
//! E10.1 / RT-02 slice (partial, packages remain open):
//! - RT-02.01 partial — reuse RT-01 leases/ledger; no third scheduler.
//! - RT-02.02 / E10.1 — reserve before construction; 512 MiB ordinary ceiling;
//!   production [`MAX_CELLS`] is 16 (was a four-slot demonstration table).
...
//!
//! Caller-backed descriptor admission lives in [`super::descriptors`]. The
//! four-slot demonstration table is gone: [`CellTable`] is the production table.
//!
//! Not in this slice: RT-02.04 measurement split, E10.5 scaling, E10.6 AF_XDP.
...
/// Occupied cell slots (E10.1 production table). A 17th is [`QdnfError::Capacity`].
...
/// Sixteen occupancy flags plus owner-held reservation handles (E10.1).
pub struct CellTable {
    occupied: [bool; MAX_CELLS],
    charges: [ChargeRef; MAX_CELLS],
}
...
impl CellTable {
    pub const fn new() -> Self {
        Self {
            ...
            charges: [ChargeRef::EMPTY; MAX_CELLS],
        }
    }
    ...
    pub fn reserve(
        ...
        self.charges[id as usize] = charge.as_ref();
        Ok(CellSlot {
            id,
            profile,
            bytes,
    ...
    pub fn release(
        ...
        if charge == ChargeRef::EMPTY {
            return Err(QdnfError::DoubleRelease);
        }
        self.charges[idx] = ChargeRef::EMPTY;
        let _ = slot.bytes;
        ledger.release(ReservationHandle::from_ref(charge))?;
...
mod tests {
    ...
    #[test]
    fn sixteen_cells_ok_seventeenth_capacity() {
        let mut table = CellTable::new();
        let mut leases = LeaseTable::new();
        let mut ledger = cell_ledger();
        let mut i = 0usize;
        while i < MAX_CELLS {
            table
                ...
                .expect("cell within 16");
            i += 1;
        }
        assert_eq!(table.occupied(), MAX_CELLS);
        assert_eq!(leases.occupied_count(), MAX_CELLS);
        assert_eq!(
            table.reserve(&mut leases, &mut ledger, CellProfile::Ordinary, ORDINARY_64,),
            Err(QdnfError::Capacity)
        );
        assert_eq!(table.occupied(), MAX_CELLS);
        assert_eq!(ledger.used().cell.bytes, ORDINARY_64 * MAX_CELLS as u64);
    }
