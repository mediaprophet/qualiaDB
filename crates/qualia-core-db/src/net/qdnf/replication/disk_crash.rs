...
//! [`disk_backend_crash_injected`] stays false: the tempfile adapter is not a
//! disk WAL injector.
//!
//! Tiny hook: [`FilePairStore::effect_path`] lets [`super::stream`] page the
//! effect file without loading a second copy of the replica.
...
/// Max effect payload retained on this path. One stream page.
pub const MAX_EFFECT_BYTES: usize = 4096;

/// Durability class actually achieved on this tempfile adapter.
#[repr(u8)]
...
#[inline]
pub const fn disk_backend_crash_injected() -> bool {
    false
}
...
impl FilePairStore {
    ...
    /// Effect file path for caller-paged verification ([`super::stream`]).
    #[inline]
    pub fn effect_path(&self) -> PathBuf {
        self.path("effect")
    }
...
mod tests {
    use super::*;
    use crate::net::qdnf::replication::stream::{hash_file_pages, PageWindow, PagedFile};
    ...
    #[test]
    fn effect_path_pages_without_second_replica_copy() {
        let dir = TempDir::new().unwrap();
        let store = FilePairStore::open(dir.path()).unwrap();
        let bytes = [0x5Au8; 64];
        let digest = store.write_effect(&bytes).unwrap();
        let mut file = PagedFile::open(store.effect_path()).unwrap();
        let mut window = PageWindow::new();
        let mut page_digests = [StrongDigest::ZERO; 1];
        let (got, pages) = hash_file_pages(&mut file, &mut window, &mut page_digests).unwrap();
        assert_eq!(got, digest);
        assert_eq!(pages, 1);
        assert!(window.peak_bytes() <= MAX_EFFECT_BYTES);
        assert!(window.peak_bytes() <= crate::net::qdnf::replication::stream::PAGE_BYTES);
    }
}
