...
impl<'a> QueryExecutor<'a> {
    ...
    fn execute_operator(
        ...
        match operator.operator_type {
            ...
            PhysicalOperatorType::Sort { input, .. } => {
                self.execute_sort(input, plan, ctx, row, results)
            }
    ...
    fn execute_hash_join(
        ...
        // For now, use nested loop join (simplified)
        self.execute_nested_loop_join(left, right, join_var, plan, ctx, row, results)
    }
    ...
    fn execute_nested_loop_join(
        ...
        // Simple cross product (for now - TODO: implement actual join)
        for l in &left_results {
            for r in &right_results {
                let mut joined = BindingRow::new();
                // Merge bindings
                for i in 0..MAX_BINDINGS {
                    if l.slots[i].is_some() {
                        joined.slots[i] = l.slots[i];
                    } else if r.slots[i].is_some() {
                        joined.slots[i] = r.slots[i];
                    }
                }
                results.push(joined);
            }
    ...
    fn execute_sort(
        ...
        // Simple bubble sort (for production, use proper sorting algorithm)
        let n = all_results.len();
        for i in 0..n {
            for j in 0..n - i - 1 {
                // Compare by first slot (simplified)
                if let (Some(a), Some(b)) = (all_results[j].slots[0], all_results[j + 1].slots[0]) {
                    if a > b {
                        all_results.swap(j, j + 1);
                    }
                }
            }
        }

        results.extend_from_slice(&all_results);
        Ok(!results.is_empty())
    }
    ...
    fn execute_union(
        ...
        // Execute left
        self.execute_operator(left, plan, ctx, row, &mut all_results)?;
        
        // Execute right
        self.execute_operator(right, plan, ctx, row, &mut all_results)?;
        
        // Remove duplicates using simple linear scan
    ...
    fn execute_optional(
        ...
        for left_result in left_results {
            let mut right_results = Vec::new();
            let mut right_row = left_result.clone(); // Copy left bindings
    ...
    fn execute_group_by(
        ...
        let mut agg_ctx = AggregationContext::new();
        ...
        for result in &all_results {
            ...
            let group_idx = agg_ctx.find_or_create_group(key, AggregateFunction::Count)?;
            agg_ctx.add_value_to_group(group_idx, 1); // Count for now
        }

        // Convert groups to binding rows
        for i in 0..agg_ctx.group_count as usize {
            let (key, agg) = &agg_ctx.groups[i];
            let mut result_row = BindingRow::new();
            for j in 0..key.var_count as usize {
                result_row.slots[j] = Some(key.values[j]);
            }
            // Add aggregate result to last slot
            if let Some(count) = agg.get_result() {
                result_row.slots[MAX_BINDINGS - 1] = Some(count);
            }
    ...
    fn execute_aggregate(
        &self,
        _input: OperatorId,
        _aggregates: [crate::sparql_planner::AggregateSpec; 16],
        _aggregate_count: u8,
        _plan: &ExecutionPlan,
        ...
        // For now, just return existing results (stub)
        Ok(!results.is_empty())
    }
