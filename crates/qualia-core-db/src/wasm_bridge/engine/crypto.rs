...
fn aead_seal(
    ...
    match alg.to_ascii_lowercase().as_str() {
        "aes256gcm" | "aes-256-gcm" => {
            if nonce.len() != 12 {
                return Err(JsValue::from_str(
                    "aes256gcm nonce must be 12 bytes (24 hex)",
                ));
            }
        ...
        "chacha20poly1305" | "chacha" => {
            if nonce.len() != 12 {
                return Err(JsValue::from_str(
                    "chacha20poly1305 nonce must be 12 bytes (24 hex)",
                ));
            }
            let c = chacha20poly1305::ChaCha20Poly1305::new(
                &chacha20poly1305::Key::try_from(key).unwrap(),
            );
            c.encrypt(&chacha20poly1305::Nonce::try_from(nonce).unwrap(), payload)
                .map_err(|_| fail())
        }
        "xchacha20poly1305" | "xchacha" => {
            if nonce.len() != 24 {
                return Err(JsValue::from_str(
                    "xchacha20poly1305 nonce must be 24 bytes (48 hex)",
                ));
            }
            let c = chacha20poly1305::XChaCha20Poly1305::new(
                &chacha20poly1305::Key::try_from(key).unwrap(),
            );
            c.encrypt(&chacha20poly1305::XNonce::try_from(nonce).unwrap(), payload)
                .map_err(|_| fail())
        }
...
fn aead_open(
    ...
    match alg.to_ascii_lowercase().as_str() {
        ...
        "chacha20poly1305" | "chacha" => {
            ...
            c.decrypt(&chacha20poly1305::Nonce::try_from(nonce).unwrap(), payload)
                .map_err(|_| fail())
        }
        "xchacha20poly1305" | "xchacha" => {
            if nonce.len() != 24 {
            ...
            c.decrypt(&chacha20poly1305::XNonce::try_from(nonce).unwrap(), payload)
                .map_err(|_| fail())
        }
        other => Err(JsValue::from_str(&format!(
            "unknown algorithm '{other}' (aes256gcm | chacha20poly1305 | xchacha20poly1305)"
