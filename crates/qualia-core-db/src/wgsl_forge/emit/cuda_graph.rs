        ...
        if let OpNode::MatMul { tc, trans_b, .. } = node.op {
            if trans_b {
                return Err(ForgeError::Emission(
                    "CudaCLowerer::matmul: trans_b not supported (transpose B host-side)".to_string(),
                ));
            }
            // tc=true → genuine NVIDIA tensor cores (WMMA, f16 in / f32 acc); else plain f32.
            self.source
                .push_str(if tc { WMMA_GEMM_TILED_SRC } else { GEMM_F32_SRC });
            self.source.push('\n');
            Ok(())
        } else {
...
pub fn emit_graph_cuda_c(
    ...
    let mut lowerer = CudaCLowerer { source: &mut source };
...
mod tests {
    ...
    fn matmul_graph_lowers_to_cuda_c() {
        ...
        for (tc, entry, marker) in [
            ...
            let out = g
                .push(
                    OpNode::MatMul { m: 0, n: 0, k: 0, tc, trans_b: false },
                    &[a, b],
                    dyn2,
                    DType::F32,
                    Schedule::default(),
    ...
    fn unsupported_matmul_and_ops_error() {
        ...
        let out = g
            .push(
                OpNode::MatMul { m: 0, n: 0, k: 0, tc: false, trans_b: true },
                &[a, b],
                dyn2,
                DType::F32,
                Schedule::default(),
...
mod cuda_tests {
    ...
    fn run_graph_gemm(
        ...
        let (view_a, view_b) = if is_f16 {
            let ab: Vec<u16> = a.iter().map(|&x| half::f16::from_f32(x).to_bits()).collect();
            let bb: Vec<u16> = b.iter().map(|&x| half::f16::from_f32(x).to_bits()).collect();
            (
                ctx.allocate_and_write(bytemuck::cast_slice(&ab), 0, 0).unwrap(),
                ctx.allocate_and_write(bytemuck::cast_slice(&bb), 1, 0).unwrap(),
            )
        } else {
            (
                ctx.allocate_and_write(bytemuck::cast_slice(a), 0, 0).unwrap(),
                ctx.allocate_and_write(bytemuck::cast_slice(b), 1, 0).unwrap(),
            )
        };
        let zeros = vec![0.0f32; m * n];
        let view_c = ctx.allocate_and_write(bytemuck::cast_slice(&zeros), 2, 0).unwrap();
        let dims: [u32; 3] = [m as u32, n as u32, k as u32];
        let view_dims = ctx.allocate_and_write(bytemuck::cast_slice(&dims), 3, 0).unwrap();
        let pipeline =
        ...
        if is_f16 {
            ...
            let sched = Schedule { workgroup_size: 64, ..Default::default() };
    ...
    fn plain_matmul_graph_certifies_on_cuda() {
        ...
        for (g, w) in got.iter().zip(&want) {
            assert!((g - w).abs() <= 1e-3 + 1e-3 * w.abs(), "cuda {g} vs cpu {w}");
        }
    ...
    fn tc_matmul_graph_certifies_on_cuda_wmma() {
        ...
        for (g, w) in got.iter().zip(&want) {
            assert!((g - w).abs() <= 5e-2 + 5e-2 * w.abs(), "wmma {g} vs ref {w}");
        }
        assert!(got.iter().any(|&v| v.abs() > 1e-6));
    }
    ...
    fn kit_kernels_nvrtc_compile() {
        ...
        compile(&elementwise_cuda_c(EwKind::Silu, 64).unwrap(), "ewise_main", &[0, 1, 2]);
        compile(&elementwise_cuda_c(EwKind::Gelu, 64).unwrap(), "ewise_main", &[0, 1, 2]);
        compile(&elementwise_cuda_c(EwKind::Add, 64).unwrap(), "ewise_main", &[0, 1, 2, 3]);
        compile(&elementwise_cuda_c(EwKind::Fma, 64).unwrap(), "ewise_main", &[0, 1, 2, 3, 4]);
