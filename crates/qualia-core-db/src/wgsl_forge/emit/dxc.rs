...
pub fn compile_hlsl_to_spirv(hlsl_source: &str, entry_point: &str) -> Result<Vec<u8>, ForgeError> {
    let mut temp_in = NamedTempFile::new()
        .map_err(|e| ForgeError::Emission(format!("Failed to create temp input file: {:?}", e)))?;
    temp_in
        .write_all(hlsl_source.as_bytes())
        .map_err(|e| ForgeError::Emission(format!("Failed to write HLSL: {:?}", e)))?;

    let temp_out = NamedTempFile::new()
        .map_err(|e| ForgeError::Emission(format!("Failed to create temp output file: {:?}", e)))?;
