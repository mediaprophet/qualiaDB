...
fn emit_kernel_body(
    ...
    if matches!(kernel.id.as_str(), "gemm" | "gemv" | "fft") {
        ...
        let mut lowerer = WgslDelegateLowerer {
            source,
            kernel,
            schedule,
        };
    ...
    if kernel.id == "ray-probe" {
        ...
        writeln!(source, "enable wgpu_ray_query;")
            .map_err(|error| ForgeError::Emission(error.to_string()))?;
    }
...
struct AffineParams {{
    ...
    for buffer in &kernel.buffers {
        ...
        let type_decl = match buffer.element {
            ...
            crate::wgsl_forge::ir::BufferElement::Scalar(
                crate::wgsl_forge::ir::ScalarType::U32,
            ) => "array<u32>",
            crate::wgsl_forge::ir::BufferElement::Scalar(
                crate::wgsl_forge::ir::ScalarType::I32,
            ) => "array<i32>",
            crate::wgsl_forge::ir::BufferElement::Scalar(
                crate::wgsl_forge::ir::ScalarType::U64Words,
            ) => "array<vec2<u32>>",
            crate::wgsl_forge::ir::BufferElement::AffineParams => "AffineParams",
            crate::wgsl_forge::ir::BufferElement::P64Words64 => "array<P64Words64>",
            crate::wgsl_forge::ir::BufferElement::AccelerationStructure => {
...
fn {}(@builtin(global_invocation_id) gid: vec3<u32>) {{"#,
    ...
    if kernel.id == "ray-probe" {
        ...
        writeln!(source, "    let i = gid.x;")
            .map_err(|error| ForgeError::Emission(error.to_string()))?;
        writeln!(source, "    if (i >= arrayLength(&hits)) {{ return; }}")
            .map_err(|error| ForgeError::Emission(error.to_string()))?;
        writeln!(source, "    let base = i * 8u;")
            .map_err(|error| ForgeError::Emission(error.to_string()))?;
        writeln!(
            source,
            "    let origin = vec3<f32>(rays[base], rays[base + 1u], rays[base + 2u]);"
        )
        .map_err(|error| ForgeError::Emission(error.to_string()))?;
        writeln!(
            source,
            "    let direction = vec3<f32>(rays[base + 3u], rays[base + 4u], rays[base + 5u]);"
        )
        .map_err(|error| ForgeError::Emission(error.to_string()))?;
        writeln!(source, "    let t_min = rays[base + 6u];")
            .map_err(|error| ForgeError::Emission(error.to_string()))?;
        writeln!(source, "    let t_max = rays[base + 7u];")
            .map_err(|error| ForgeError::Emission(error.to_string()))?;
        emit_ops(source, &kernel.ops, "    ")?;
        writeln!(source, "    hits[i] = hit_t;")
            .map_err(|error| ForgeError::Emission(error.to_string()))?;
        writeln!(source, "}}").map_err(|error| ForgeError::Emission(error.to_string()))?;
        ...
        if schedule.vector_width == 1 {
            ...
            writeln!(
                source,
                "            let value = vec{}<f32>({});",
                schedule.vector_width, constructor
            )
            .map_err(|error| ForgeError::Emission(error.to_string()))?;
            ...
            for index in 0..schedule.vector_width {
                ...
                writeln!(
                    source,
                    "            output[global_id + {index}u] = transformed.{};",
            ...
            writeln!(
                source,
                "                let global_id = global_id + component;"
            )
            .map_err(|error| ForgeError::Emission(error.to_string()))?;
            writeln!(source, "                if (global_id < params.length) {{")
                .map_err(|error| ForgeError::Emission(error.to_string()))?;
...
impl crate::wgsl_forge::ir::graph::Lowerer for WgslDelegateLowerer<'_> {
    ...
    fn gemv(&mut self, _node: &crate::wgsl_forge::ir::graph::GraphNode) -> Result<(), ForgeError> {
        emit_gemv_wgsl(self.source, self.kernel, self.schedule)
    }
    fn fft(&mut self, _node: &crate::wgsl_forge::ir::graph::GraphNode) -> Result<(), ForgeError> {
...
impl crate::wgsl_forge::ir::graph::Lowerer for WgslGraphLowerer<'_> {
    fn reduce(&mut self, node: &crate::wgsl_forge::ir::graph::GraphNode) -> Result<(), ForgeError> {
        use crate::wgsl_forge::ir::graph::OpNode;
        if let OpNode::Reduce { op, .. } = node.op {
            self.source
                .push_str(&crate::wgsl_forge::graph_ops::reduce::reduce_wgsl(
                    op,
...
pub fn emit_graph_wgsl(
    ...
    .map_err(|error| ForgeError::Emission(error.to_string()))?;
    let mut lowerer = WgslGraphLowerer {
        source: &mut source,
    };
    ...
    for op in ops {
        match op {
            Op::StructLoad {
                ...
                writeln!(source, "{indent}let {destination} = {buffer}.{field};")
                    .map_err(|error| ForgeError::Emission(error.to_string()))?;
            }
            Op::Load {
                buffer,
                ...
                writeln!(source, "{indent}let {destination} = {buffer}[{index}];")
                    .map_err(|error| ForgeError::Emission(error.to_string()))?;
            }
            Op::Store {
                buffer,
            ...
            Op::Fma {
                ...
                writeln!(source, "{indent}let {destination} = {a} * {b} + {c};")
                    .map_err(|error| ForgeError::Emission(error.to_string()))?;
            }
            Op::Mul {
                left,
                ...
                writeln!(source, "{indent}let {destination} = {left} * {right};")
                    .map_err(|error| ForgeError::Emission(error.to_string()))?;
            }
            Op::Add {
                left,
            ...
            Op::DotProduct {
                ...
                writeln!(source, "{indent}var {destination} = 0.0;")
                    .map_err(|error| ForgeError::Emission(error.to_string()))?;
                writeln!(
                    source,
                    "{indent}for (var i: u32 = 0u; i < {len}; i = i + 1u) {{"
                )
                .map_err(|error| ForgeError::Emission(error.to_string()))?;
            ...
            Op::Relu {
                ...
                writeln!(source, "{indent}let {destination} = max(0.0, {operand});")
                    .map_err(|error| ForgeError::Emission(error.to_string()))?;
            }
            Op::Gelu {
                operand,
            ...
            Op::Barrier => {
                writeln!(source, "{indent}workgroupBarrier();")
                    .map_err(|error| ForgeError::Emission(error.to_string()))?;
            }
            Op::Intrinsic(crate::wgsl_forge::ir::Intrinsic::RayQuery {
                ...
                writeln!(source, "{indent}var rq: ray_query;")
                    .map_err(|error| ForgeError::Emission(error.to_string()))?;
                ...
                writeln!(
                    source,
                    "{indent}loop {{ if (!rayQueryProceed(&rq)) {{ break; }} }}"
                )
                .map_err(|error| ForgeError::Emission(error.to_string()))?;
                writeln!(
                    source,
                    "{indent}let committed = rayQueryGetCommittedIntersection(&rq);"
                )
                .map_err(|error| ForgeError::Emission(error.to_string()))?;
                writeln!(source, "{indent}var {destination} = -1.0;")
                    .map_err(|error| ForgeError::Emission(error.to_string()))?;
            ...
            Op::Intrinsic(_) => {
                return Err(ForgeError::Emission(
                    "Intrinsics not implemented for WGSL yet".to_string(),
                ));
            }
        }
    }
