...
/// WGSL. Later phases replace these delegations with native graph-template emission and
/// add the remaining op-class methods; until then, unimplemented op-classes inherit the
/// trait's default `Err` (never a silent no-op).
