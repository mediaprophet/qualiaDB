...
pub struct AggregationContext {
    pub groups: [(GroupKey, [AggregateAccumulator; 16]); 64], // Max 64 groups
    pub group_count: u8,
    pub aggregates_spec: [crate::sparql_planner::AggregateSpec; 16],
    pub aggregate_count: u8,
}
...
impl AggregationContext {
    ...
    pub fn find_or_create_group(&mut self, key: GroupKey) -> Result<usize, String> {
        ...
        for i in 0..self.aggregate_count as usize {
            let func = match self.aggregates_spec[i].func {
                0 => AggregateFunction::Count,
                1 => AggregateFunction::Sum,
                2 => AggregateFunction::Avg,
                3 => AggregateFunction::Min,
            ...
            accumulators[i] = AggregateAccumulator::new(func);
        }
        self.groups[idx] = (key, accumulators);
        self.group_count += 1;
        Ok(idx)
    }

    pub fn add_values_to_group(&mut self, group_idx: usize, row: &BindingRow) {
        for i in 0..self.aggregate_count as usize {
            let spec = &self.aggregates_spec[i];
            if spec.func == 0 {
                // COUNT(*) usually just increments
                self.groups[group_idx].1[i].add_value(0);
            } else if let Some(val) = row.get(spec.input_var) {
                self.groups[group_idx].1[i].add_value(val);
            }
        }
    }
}
