...
pub fn serialize_quins_to_json<W: Write>(
    ...
    writeln!(writer, "[")
        .map_err(|e| format!("Failed to write JSON array start: {}", e))?;
    ...
    for (_subject, row_quins) in subjects {
        if !first {
            writeln!(writer, ",")
                .map_err(|e| format!("Failed to write JSON comma: {}", e))?;
        }
        first = false;

        write!(writer, "  {{")
            .map_err(|e| format!("Failed to write JSON object start: {}", e))?;

        let mut field_first = true;
        for (i, &pred_hash) in profile.predicate_hashes.iter().enumerate() {
            let field_name = &profile.field_names[i];
            let datatype = profile.datatypes.get(i).copied().unwrap_or(JsonDatatype::StringRef);
            ...
            if let Some(quin) = row_quins.iter().find(|q| q.predicate == pred_hash) {
                ...
                write!(writer, "\"{}\": {}", field_name, format_quin_value(quin, datatype))
                    .map_err(|e| format!("Failed to write JSON field: {}", e))?;
            }
        }

        write!(writer, "}}")
            .map_err(|e| format!("Failed to write JSON object end: {}", e))?;
    }
