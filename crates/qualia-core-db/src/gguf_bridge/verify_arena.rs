...
impl QTensorEngine {
    ...
    fn run_verify_batch(
        ...
        for (t, &tok) in tokens.iter().enumerate() {
            let n = index.dequantize_token_embedding_into(&mmap, tok, &mut emb[t * n_embd..(t + 1) * n_embd]);
            if n == 0 {
                return None;
            }
        }
    ...
    fn build_verify_plan(
        ...
        let logits_out = mk_storage("VerifyLogitsOut", bmax * vocab, wgpu::BufferUsages::COPY_SRC);
        ...
        let mk_gemm_bg = |input: &wgpu::Buffer,
            ...
            device.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                entries: &[
                    ...
                    wgpu::BindGroupEntry { binding: 3, resource: out_res },
                ],
            })
        };
        ...
        for l in 0..n_layer {
            ...
            layers.push(VerifyLayerBinds {
                ...
                k_gemm: mk_gemm_bg(&normed, k_w.as_entire_binding(), slot_off(lu, S_GEMM_K), kv_proj.as_entire_binding()),
                k_write: mk_attn_bg(&kv_proj, &k_w, slot_off(lu, S_ATTN_KW), l, &attn_out),
                v_gemm: mk_gemm_bg(&normed, v_w.as_entire_binding(), slot_off(lu, S_GEMM_V), kv_proj.as_entire_binding()),
                v_write: mk_attn_bg(&kv_proj, &v_w, slot_off(lu, S_ATTN_VW), l, &attn_out),
                q: mk_attn_bg(&normed, &q_w, slot_off(lu, S_ATTN_Q), l, &attn_out),
                o: mk_gemm_bg(&attn_out, o_w.as_entire_binding(), slot_off(lu, S_GEMM_O), delta.as_entire_binding()),
                ...
                gate: mk_gemm_bg(&normed, g_w.as_entire_binding(), slot_off(lu, S_GEMM_GATE), gate_buf.as_entire_binding()),
                up: mk_gemm_bg(&normed, u_w.as_entire_binding(), slot_off(lu, S_GEMM_UP), up_buf.as_entire_binding()),
                silu: mk_elem_bg(&silu_layout, gate_buf.as_entire_binding(), up_buf.as_entire_binding(), &silu_buf, slot_off(lu, S_ELEM_SILU)),
                down: mk_gemm_bg(&silu_buf, d_w.as_entire_binding(), slot_off(lu, S_GEMM_DOWN), delta.as_entire_binding()),
