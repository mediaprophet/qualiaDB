...
impl QTensorEngine {
    ...
    /// Same as [`Self::mc8_upload_all_resident_weights`] but yields to the browser every layer
    /// so phones can paint "Uploading layer k/n" instead of freezing on Initialising WebGPU.
    #[cfg(target_arch = "wasm32")]
    pub(crate) async fn mc8_upload_all_resident_weights_yielding(
        &mut self,
        index: &crate::gguf_sharder::GgufTensorIndex,
    ) -> bool {
        use crate::gguf_bridge::wasm_yield::{set_init_status, yield_to_browser};

        if self.mc8_weights_resident {
            return true;
        }
        // Reuse the sync setup path for buffer allocation, then upload with yields.
        // We cannot simply call the sync uploader: it monopolises the main thread.
        let mmap_arc = match self.gguf_mmap.clone() {
            Some(a) => a,
            None => return false,
        };
        ...
        set_init_status(format!(
            "Allocating {:.0} MB GPU weight buffers…",
            total_bytes as f64 / (1024.0 * 1024.0)
        ));
        ...
        for layer in 0..n_layer {
            set_init_status(format!(
                "Uploading weights to GPU — layer {}/{}",
                layer + 1,
                n_layer
            ));
            // Yield every layer so the status line advances on phones.
            yield_to_browser().await;
            let t = index.get_layer_tensors(layer);
            let queue = self.gpu_queue();
            for role in ROLES {
        ...
        self.mc8_bg_cache.lock().ok().map(|mut c| c.clear());
        wlog(&format!(
            "[MC8] resident weights uploaded (yielding): {:.1} MB across {} layers",
            total_bytes as f64 / (1024.0 * 1024.0),
            n_layer
        ));
        true
    ...
    pub(crate) fn adopt_resident_p64(&mut self, data: Arc<[u8]>) -> Result<(), String> {
        // Sync path used by native tests; WASM prefers `adopt_resident_p64_async`.
        self.adopt_resident_p64_inner(data, /*defer_weight_upload=*/ false)
    }

    /// Async P64 boot for the browser: yields between phases and can **defer** the multi‑hundred‑MB
    /// GPU weight upload so phones leave "Initialising WebGPU" instead of freezing forever.
    #[cfg(target_arch = "wasm32")]
    pub(crate) async fn adopt_resident_p64_async(
        &mut self,
        data: Arc<[u8]>,
        ...
        phase(&format!(
            "Reserving KV / GEMM arenas ({} layers, embd {})…",
            hp.n_layer, hp.n_embd
        ))
        .await;
        self.hyperparams = hp;
        ...
        if self.kv_layout.is_none() || self.kv_cache_cpu.is_none() {
            return Err(
                "P64: KV cache allocation failed (device out of memory? close other tabs, use 360M only)"
                    .to_string(),
            );
        }
        ...
        if defer_weight_upload {
            // Phone path: finish init without materialising ~200–400 MB of GPU buffers.
            // First forward pass uploads lazily (slower TTFT, but the UI becomes ready).
            phase(
                "Skipping eager GPU weight upload (phone path) — first reply will be slower…",
            )
            .await;
            wlog("[P64] defer_weight_upload=true — resident weights deferred to first forward");
            wlog(&format!(
                "[P64] boot OK (deferred weights): {} tensors, {} layers",
                q.entries.len(),
                hp.n_layer
            ));
            return Ok(());
    ...
    fn adopt_resident_p64_inner(
        ...
        self.p64_resident = Some(data);
        if defer_weight_upload {
            wlog("[P64] defer_weight_upload=true — resident weights deferred");
            return Ok(());
        }
