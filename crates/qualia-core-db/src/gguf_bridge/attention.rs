...
impl QTensorEngine {
    ...
    /// Native fused attention tail: Q-SDPA writes into `gemm_output_buf`, then `o_proj` consumes that
    /// GPU buffer in the same command buffer and only the projected residual is read back. This keeps
    /// the old CPU round-trip path as the fallback while avoiding the cross-submit visibility issue
    /// seen when Q and `o_proj` were submitted separately.
    #[cfg(not(target_arch = "wasm32"))]
    pub(crate) fn dispatch_attention_q_o_fused(
        ...
        let attn_layout = self.attention_pipeline.get_bind_group_layout(0);
    ...
    pub(crate) fn dispatch_attention_layer(
        ...
        #[cfg(not(target_arch = "wasm32"))]
        if crate::llm_bench::attention_preproject_enabled() && crate::llm_bench::coop_gemv_enabled() {
            ...
            if kv_dim > 0
                ...
                && v_out == kv_dim
                && q_out == q_dim
                && kv_dim <= scratch_a.len()
                && q_dim <= scratch_a.len()
                && q_dim <= scratch_b.len()
            {
                if self.dispatch_gemm_into(index, k_info, &hidden_input[..k_in], &mut scratch_a[..kv_dim], k_in, kv_dim)
                    && self.dispatch_attention_pass(
                        &scratch_a[..kv_dim],
                        kv_dim,
                        1,
                        token_idx,
                    ...
                    && self.dispatch_gemm_into(index, v_info, &hidden_input[..v_in], &mut scratch_a[..kv_dim], v_in, kv_dim)
                    && self.dispatch_attention_pass(
                        &scratch_a[..kv_dim],
                        kv_dim,
                        1,
                    ...
                    && self.dispatch_gemm_into(index, q_info, &hidden_input[..q_in], &mut scratch_a[..q_dim], q_in, q_dim)
                    && self.dispatch_attention_pass(
                        &scratch_a[..q_dim],
                        q_dim,
                        1,
        ...
        if crate::llm_bench::attention_o_fuse_enabled() {
            if let Some(out_info) = tensors.attn_output {
                ...
                if o_in <= q_dim && o_out <= scratch_a.len() {
                    if let Ok(o_raw) =
                        crate::ggml_quants::fetch_tensor_bytes(mmap, index.tensor_data_start, &out_info)
                    {
