...
impl Default for VerifyPolicy {
    fn default() -> Self {
        Self { freivalds_rounds: 16, tol: 1e-6, seed: 0xF1E1, min_mrr: 0.9 }
    }
}
...
pub fn verify(input: &JobInput, result: &JobResult, policy: VerifyPolicy) -> VerificationVerdict {
    match (input, result) {
        (
            ...
            if c.len() != m * n {
                return VerificationVerdict::Rejected { reason: "result has wrong dimensions" };
            }
            verify_product(*m, *k, *n, a, b, c, policy)
        }
        (
            JobInput::EmbeddingArtifact { n_entities, check, .. },
            JobResult::EmbeddingArtifact { table },
        ) => verify_artifact(table, check, *n_entities, policy.min_mrr),
        _ => VerificationVerdict::Rejected { reason: "result kind does not match job kind" },
    }
}
...
fn verify_product(
    ...
    for _ in 0..policy.freivalds_rounds.max(1) {
        ...
        if matvec(Transpose::No, k, n, b, &x, &mut bx).is_err() {
            return VerificationVerdict::Rejected { reason: "reference matvec failed" };
        }
        // abx = A·(Bx)  (A is m×k)
        ...
        for i in 0..m {
            if (abx[i] - cx[i]).abs() > policy.tol {
                return VerificationVerdict::Rejected { reason: "A·B ≠ C (Freivalds)" };
            }
        }
    }
...
fn verify_artifact(
    ...
    if check.is_empty() {
        return VerificationVerdict::Rejected { reason: "no check set to verify the artifact" };
    }
    let mut recip_sum = 0.0;
    for &(h, r, t) in check {
        let target = match table.score(h, r, t) {
            Ok(s) => s,
            Err(_) => return VerificationVerdict::Rejected { reason: "artifact could not be scored" },
        };
    ...
    if mrr >= min_mrr {
        ...
        VerificationVerdict::Rejected { reason: "artifact MRR below the acceptance floor" }
    }
}
...
mod tests {
    ...
    fn freivalds_accepts_a_correct_product() {
        ...
        let good = JobResult::DenseLinearProduct { c: vec![19.0, 22.0, 43.0, 50.0] };
    ...
    fn freivalds_rejects_a_wrong_product() {
        ...
        // One entry corrupted.
        let bad = JobResult::DenseLinearProduct { c: vec![19.0, 22.0, 43.0, 999.0] };
    ...
    fn freivalds_rejects_a_subtly_wrong_product() {
        ...
        let input = JobInput::DenseLinearProduct { m: n, k: n, n, a, b };
    ...
    fn artifact_verification_accepts_a_learned_table_and_rejects_garbage() {
        ...
        let cfg = TrainConfig {
            model: ScoreModel::TransE { p: 2 }, rank: 8, epochs: 400, lr: 0.05,
            margin: 1.0, reg: 0.0, neg_per_pos: 4, seed: 7,
        };
        let table = train(&triples, 4, 1, cfg).unwrap();
        let input = JobInput::EmbeddingArtifact {
            triples, n_entities: 4, n_relations: 1, cfg, check: vec![(0, 0, 1), (2, 0, 3)],
        };
    ...
    fn mismatched_kinds_rejected() {
        let input = JobInput::DenseLinearProduct { m: 1, k: 1, n: 1, a: vec![1.0], b: vec![1.0] };
        let table = crate::solvers::learning::kg_embedding::EmbeddingTable::zeros(
            ScoreModel::TransE { p: 2 }, 2, 2, 1,
        ).unwrap();
