...
fn coop_gemv_dual(
    ...
    // [WASM] Shared-memory tree reduction (replaces subgroupAdd).
    partial_a[t] = acc_a;
    partial_b[t] = acc_b;
    workgroupBarrier();
    for (var stride = COOP_WG / 2u; stride > 0u; stride = stride >> 1u) {
        if t < stride {
            partial_a[t] = partial_a[t] + partial_a[t + stride];
            partial_b[t] = partial_b[t] + partial_b[t + stride];
        }
        workgroupBarrier();
    }
    if t == 0u {
        out_a[out_base + row] = partial_a[0];
        out_b[out_base + row] = partial_b[0];
    }
}
...
fn coop_gemv_dual_mr(
    ...
    // [WASM] Per-row shared-memory tree reduce + write (replaces subgroupAdd).
    var aa: array<f32, 4>;
    var bb: array<f32, 4>;
