    ...
    pub fn flush_block(
        ...
        #[cfg(target_family = "windows")]
        {
            ...
            while written < BLOCK_MULTIPLIER_SIZE {
                let n = self
                    .file
                    .seek_write(&bytes[written..], self.current_offset + written as u64)?;
                if n == 0 {
...
mod tests {
    ...
    fn test_flush_superblock() {
        ...
        assert_eq!(
            metadata.len(),
            BLOCK_MULTIPLIER_SIZE as u64,
            "File size is not page aligned to 40,960 bytes"
        );
    }
}
...
impl VirtualFileSystem for NativeVfs {
    fn read_chunk(
        ...
        Box::pin(async move { std::fs::read(&path).map_err(|e| e.to_string()) })
    }
    ...
    fn write_chunk(
        ...
        Box::pin(async move { std::fs::write(&path, data).map_err(|e| e.to_string()) })
    }
}
...
impl VirtualFileSystem for OpfsVfs {
    fn read_chunk(&self, path: &str) -> Pin<Box<dyn Future<Output = Result<Vec<u8>, String>>>> {
        ...
        Box::pin(async move {
            ...
            let dir_handle_val = JsFuture::from(storage.get_directory())
                .await
                .map_err(|e| format!("{:?}", e))?;
            let dir_handle: web_sys::FileSystemDirectoryHandle = dir_handle_val.unchecked_into();

            let file_handle_val = JsFuture::from(dir_handle.get_file_handle(&path))
                .await
                .map_err(|e| format!("{:?}", e))?;
            ...
            let array_buffer_val = JsFuture::from(file.array_buffer())
                .await
                .map_err(|e| format!("{:?}", e))?;
    ...
    fn write_chunk(
        ...
        Box::pin(async move {
            ...
            let file_handle_val =
                JsFuture::from(dir_handle.get_file_handle_with_options(&path, &options))
                    .await
                    .map_err(|e| format!("{:?}", e))?;
            let file_handle: web_sys::FileSystemFileHandle = file_handle_val.unchecked_into();
