...
pub fn is_contained_in_parent(
    ...
    global_pt[0] >= parent_bounds_min[0]
        && global_pt[0] <= parent_bounds_max[0]
        && global_pt[1] >= parent_bounds_min[1]
        && global_pt[1] <= parent_bounds_max[1]
        && global_pt[2] >= parent_bounds_min[2]
        && global_pt[2] <= parent_bounds_max[2]
}
...
mod tests {
    ...
    fn test_is_contained_in_parent() {
        ...
        assert!(is_contained_in_parent(
            local_pt, &frame, min_bounds, max_bounds
        ));

        // Outside parent bounds
        let out_bounds = [0.0, 0.0, 1.0];
        assert!(!is_contained_in_parent(
            local_pt, &frame, min_bounds, out_bounds
        ));
    }
}
