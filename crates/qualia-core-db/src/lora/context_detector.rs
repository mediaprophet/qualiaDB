...
impl ContextDetector {
    pub fn new() -> Self {
        ...
        for w in [
            ...
            add!(
                w,
                (ContextType::Medical, 0.9),
                (ContextType::Biological, 0.4)
            );
        }
        ...
        for w in [
            ...
            add!(
                w,
                (ContextType::Biological, 0.9),
                (ContextType::Chemical, 0.3)
            );
        }
    ...
    pub fn combine(
        ...
        if text_result.1 > meta_result.1 {
            text_result
        } else {
            meta_result
        }
...
mod tests {
    ...
    fn test_medical_detection() {
        ...
        assert_eq!(
            ctx,
            ContextType::Medical,
            "expected Medical, got {:?} (conf={conf:.2})",
            ctx
        );
        assert!(conf > 0.5);
    }
    ...
    fn test_legal_detection() {
        ...
        assert_eq!(
            ctx,
            ContextType::Legal,
            "expected Legal, got {:?} (conf={conf:.2})",
            ctx
        );
        assert!(conf > 0.5);
    }
    ...
    fn test_chemical_detection() {
        let det = ContextDetector::new();
        let (ctx, conf) =
            det.analyze_text("Organic synthesis of the catalyst compound via reaction");
        assert_eq!(
            ctx,
            ContextType::Chemical,
            "expected Chemical, got {:?} (conf={conf:.2})",
            ctx
        );
        assert!(conf > 0.4);
    }
    ...
    fn test_biological_detection() {
        let det = ContextDetector::new();
        let (ctx, conf) =
            det.analyze_text("Gene expression in the cell affects protein synthesis via RNA");
        assert_eq!(
            ctx,
            ContextType::Biological,
            "expected Biological, got {:?} (conf={conf:.2})",
            ctx
        );
        assert!(conf > 0.4);
    }
    ...
    fn test_technical_detection() {
        let det = ContextDetector::new();
        let (ctx, conf) =
            det.analyze_text("Optimizing the algorithm latency in the distributed system");
        assert_eq!(
            ctx,
            ContextType::Technical,
            "expected Technical, got {:?} (conf={conf:.2})",
            ctx
        );
        assert!(conf > 0.5);
    }
