...
fn modality_handlers_are_zero_heap() {
    ...
    // LTL (trace scan + in-place reverse)
    {
        ...
        for i in 0..3 {
            a.write_table(q(i + 1, safe, 0));
        }
        assert_eq!(
            allocs_during(&mut a, SlgOpcode::NativeLtlGlobally, prop_frame(safe)),
            0,
    ...
    // Probabilistic threshold (scan + f32 extract)
    {
        ...
        let mut bel = NQuin {
            ...
            object: o,
            context: 0,
            metadata: (0.9f32).to_bits() as u64,
            parity: 0,
        };
    ...
    // Closed-world / NAF (absence scan)
    {
        ...
        assert_eq!(
            allocs_during(
                ...
                VmFrame {
                    subject_reg: q_hash("zh:absent"),
                    predicate_reg: q_hash("zh:p"),
                    object_reg: q_hash("zh:o"),
                    context_reg: 0
                }
            ),
            0,
    ...
    // Fuzzy conjunction (degree scan)
    {
        ...
        let mut f2 = NQuin {
            ...
            object: 0,
            context: 0,
            metadata: (0.6f32).to_bits() as u64,
            parity: 0,
        };
    ...
    // Deontic (verdict scan over a stack [DeonticVerdict; N])
    {
        ...
        assert_eq!(
            allocs_during(&mut a, SlgOpcode::NativeDeonticEval, VmFrame::default()),
            0,
    ...
    // Epistemic (verdict scan over a stack [EpistemicVerdict; N])
    {
        ...
        let mut k = NQuin {
            subject: agent,
            predicate: pred,
            object: q_hash("zh:prop"),
            context: q_hash("zh:world"),
            metadata: 0,
            parity: 0,
        };
        ...
        assert_eq!(
            allocs_during(
                &mut a,
                SlgOpcode::NativeEpistemicEval(128),
                VmFrame {
                    subject_reg: agent,
                    predicate_reg: 0,
                    object_reg: 0,
                    context_reg: q_hash("zh:world")
                }
            ),
            0,
    ...
    // Defeasible (NativeUnless injects a defeater)
    {
        ...
        assert_eq!(
            allocs_during(
                ...
                VmFrame {
                    subject_reg: q_hash("zh:a"),
                    predicate_reg: (q_hash("zh:d") << 8) | 0x12,
                    object_reg: q_hash("zh:o"),
                    context_reg: q_hash("zh:c")
                }
            ),
            0,
