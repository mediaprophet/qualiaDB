...
use qualia_core_db::{
    ...
    gguf_sharder::{GgufHyperparams, GgufSharder, GgufTensorIndex, GgufTokenizer},
    llm_agent::{AgentBackend, LlmAgent, LLM_MEMORY_BUDGET_BYTES, MAX_OUTPUT_TOKENS},
};
...
impl LlmModelTester {
    ...
    fn test_single_model(
    ...
    ) -> Result<ModelTestResults, ExecutionError> {
        ...
        if let Err(e) = parsing_result {
            result
                .error_messages
                .push(format!("GGUF parsing failed: {}", e));
            result.validation_passed = false;
        }
        ...
        if let Err(e) = inference_result {
            result
                .error_messages
                .push(format!("Inference failed: {}", e));
            result.validation_passed = false;
        }
        ...
        if let Err(e) = memory_result {
            result
                .error_messages
                .push(format!("Memory test failed: {}", e));
            result.validation_passed = false;
        }
        ...
        if let Err(e) = quality_result {
            result
                .error_messages
                .push(format!("Quality test failed: {}", e));
            result.validation_passed = false;
        }
    ...
    fn test_model_loading(
        ...
        if !config.model_path.exists() {
            return Err(ExecutionError::FileNotFound(
    ...
    fn test_gguf_parsing(
        ...
        if vocab_size != config.expected_params.vocab_size {
            return Err(ExecutionError::InvalidParameters(format!(
                ...
                config.expected_params.vocab_size, vocab_size
            )));
        }
        ...
        println!(
            "    ⚙️  Hyperparameters: {} layers, {} embeddings",
    ...
    fn test_inference_performance(
        ...
        println!(
            "    📊 Average generation speed: {:.1} tokens/second",
            result.token_generation_speed
        );
    ...
    fn test_memory_constraints(
        ...
        if result.memory_usage > config.memory_limit {
            return Err(ExecutionError::MemoryLimitExceeded(format!(
                ...
                result.memory_usage, config.memory_limit
            )));
        }
        ...
        println!(
            "    ✅ Within memory limit ({:.1} MB)",
            config.memory_limit as f64 / 1024.0 / 1024.0
        );
    ...
    fn validate_hyperparameters(
        ...
        if actual.n_embd != expected.n_embd {
            return Err(ExecutionError::InvalidParameters(format!(
                ...
                expected.n_embd, actual.n_embd
            )));
        }

        if actual.n_head != expected.n_head {
            return Err(ExecutionError::InvalidParameters(format!(
                "Head count mismatch: expected {}, got {}",
                expected.n_head, actual.n_head
            )));
        }
        ...
        if actual.n_kv_head != expected.n_kv_head {
            println!(
                "    ⚠️  KV head count different: expected {}, got {} (may be GQA)",
    ...
    fn print_overall_summary(&self) {
        ...
        if !self.results.is_empty() {
            ...
            println!(
                "  Average load time: {:?}",
                avg_load_time / total_models as u32
            );
            println!(
                "  Average generation speed: {:.1} tokens/sec",
