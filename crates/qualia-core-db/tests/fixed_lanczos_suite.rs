...
fn test_lanczos_convergence() {
    ...
    assert!(result.is_ok(), "Solver should converge on a positive definite matrix");
    
    let vec = result.unwrap();
    // Normalization check
    let norm_sq = vec.data[0]*vec.data[0] + vec.data[1]*vec.data[1] + vec.data[2]*vec.data[2] + vec.data[3]*vec.data[3];
    assert!((norm_sq - 1.0).abs() < 1e-4, "Eigenvector should be normalized, got {}", norm_sq);
    ...
    assert!((lambda - 2.381966).abs() < 1e-2, "Smallest eigenvalue should be ~2.382, got {}", lambda);
    
    // Ensure Ax ≈ lambda x
    for i in 0..4 {
        assert!((ax[i] - lambda * vec.data[i]).abs() < 1e-2, "Ax != lambda x at index {}", i);
    }
}
...
fn test_lanczos_eigenvalues() {
    ...
    assert!((eigs[0] - 2.381966).abs() < 1e-2, "First eigenvalue should be ~2.382, got {}", eigs[0]);
    assert!((eigs[1] - 3.381966).abs() < 1e-2, "Second eigenvalue should be ~3.382, got {}", eigs[1]);
}
