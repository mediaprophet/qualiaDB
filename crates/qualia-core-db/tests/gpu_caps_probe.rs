...
fn gpu_caps_probe() {
    ...
    for adapter in &adapters {
        ...
        eprintln!("════════════════════════════════════════════════════════════════════");
        eprintln!("[caps] {} | {:?} | {:?}", info.name, info.backend, info.device_type);
        eprintln!("[caps] driver: {} {}", info.driver, info.driver_info);
        eprintln!("[caps] subgroup size range: {}..{}", info.subgroup_min_size, info.subgroup_max_size);
        eprintln!("[caps] ── matmul / decode features ──");
        eprintln!("[caps]   COOPERATIVE_MATRIX (tensor cores) : {}  ({} tiles)",
            yn(f.contains(F::EXPERIMENTAL_COOPERATIVE_MATRIX)), coop_tiles.len());
        eprintln!("[caps]   SUBGROUP (warp ops)               : {}", yn(f.contains(F::SUBGROUP)));
        eprintln!("[caps]   SUBGROUP_BARRIER                  : {}", yn(f.contains(F::SUBGROUP_BARRIER)));
        eprintln!("[caps]   SHADER_F16 (half precision)       : {}", yn(f.contains(F::SHADER_F16)));
        eprintln!("[caps]   SHADER_INT64                      : {}", yn(f.contains(F::SHADER_INT64)));
        eprintln!("[caps] ── profiling / misc ──");
        eprintln!("[caps]   TIMESTAMP_QUERY                   : {}", yn(f.contains(F::TIMESTAMP_QUERY)));
        eprintln!("[caps]   TIMESTAMP_QUERY_INSIDE_PASSES     : {}", yn(f.contains(F::TIMESTAMP_QUERY_INSIDE_PASSES)));
        eprintln!("[caps]   PIPELINE_STATISTICS_QUERY         : {}", yn(f.contains(F::PIPELINE_STATISTICS_QUERY)));
        eprintln!("[caps]   PIPELINE_CACHE                    : {}", yn(f.contains(F::PIPELINE_CACHE)));
        eprintln!("[caps]   EXPERIMENTAL_RAY_QUERY (RT cores) : {}", yn(f.contains(F::EXPERIMENTAL_RAY_QUERY)));
        eprintln!("[caps] ── compute limits (tile / dispatch bounds) ──");
        eprintln!("[caps]   workgroup_storage_size            : {} bytes", lim.max_compute_workgroup_storage_size);
        eprintln!("[caps]   invocations_per_workgroup         : {}", lim.max_compute_invocations_per_workgroup);
        eprintln!("[caps]   workgroup_size_x                  : {}", lim.max_compute_workgroup_size_x);
        eprintln!("[caps]   workgroups_per_dimension          : {}", lim.max_compute_workgroups_per_dimension);
        eprintln!("[caps]   storage_buffer_binding_size       : {} MiB", lim.max_storage_buffer_binding_size / (1024 * 1024));
        eprintln!("[caps]   max_buffer_size                   : {} MiB", lim.max_buffer_size / (1024 * 1024));
        for (i, t) in coop_tiles.iter().enumerate() {
            eprintln!("[caps]   coop tile[{i}]: {}x{}x{} ab={:?} cr={:?}",
    ...
    match chosen {
        Ok(a) => {
            ...
            eprintln!("[caps]   {} | {:?} | {:?}", info.name, info.backend, info.device_type);
            eprintln!("[caps]   coop_matrix={} subgroup={} f16={}  ← what decode runs on TODAY",
