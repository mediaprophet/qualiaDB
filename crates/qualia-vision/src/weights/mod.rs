...
pub enum VisionBackendKind {
    /// Heuristic grid / CPU reference â€” not production eval.
    Reference = 0,
...
pub struct VisionWeightBundle {
    pub embed_dim: usize,
    pub n_classes: usize,
    /// Projects 16-d image features â†’ embed_dim. Row-major `[embed_dim * 16]`.
    pub proj: Vec<f32>,
    pub head: LinearHead,
    pub model_id: String,
...
/// Production path: image features â†’ projection â†’ linear head â†’ whole-image or grid cells.
        ...
        for cy in 0..g {
            for cx in 0..g {
                    ...
                    .classify_embedding(&emb[..self.bundle.embed_dim], self.min_score)
                {
                    ...
                    d.flags = 0; // production path â€” not FLAG_REFERENCE_BACKEND
