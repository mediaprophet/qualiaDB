...
impl PolyPressure {
    ...
    pub fn new(channel: u8, note: u8, pressure: u8) -> Result<Self, AudioError> {
        ...
        Ok(Self { channel, note, pressure })
    }

    /// Serialize to `[0xAn, note, pressure]`.
    #[inline]
    pub fn to_bytes(self) -> [u8; 3] {
        [STATUS_POLY_PRESSURE | (self.channel & 0x0F), self.note, self.pressure]
    }
    ...
    pub fn parse(bytes: &[u8]) -> Result<Self, AudioError> {
        ...
        Ok(Self { channel: bytes[0] & 0x0F, note: bytes[1], pressure: bytes[2] })
    }
}
...
mod tests {
    ...
    #[test]
    fn rejects_bad() {
        assert_eq!(PolyPressure::new(0, 200, 0), Err(AudioError::InvalidParameter));
        assert_eq!(PolyPressure::parse(&[0xB0, 60, 80]), Err(AudioError::UnsupportedFormat));
    }
}
