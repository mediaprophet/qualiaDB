...
class AppVaultScreen extends StatefulWidget {
  const AppVaultScreen({super.key});

  @override
  State<AppVaultScreen> createState() => _AppVaultScreenState();
}

class _AppVaultScreenState extends State<AppVaultScreen> {
  ...
  Future<void> _loadInstalledApps() async {
    try {
      final names = await listInstalledApps();
      if (mounted) {
        setState(() {
          _apps
            ..clear()
            ..addAll(names.map((n) => {'id': n, 'name': n, 'status': 'Installed', 'vc': 'Valid'}));
        });
      }
    } catch (e) {
      if (mounted) setState(() => _installStatus = 'Failed to load apps: $e');
    }
  }

  Future<void> _handleLaunch(String appName) async {
    setState(() => _launchingId = appName);
    try {
      final url = await launchInstalledApp(appName: appName);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Launch failed: $e')));
      }
    } finally {
  ...
  Future<void> _handleInstallApp() async {
    final dir = await FilePicker.platform.getDirectoryPath(
      dialogTitle: 'Select app directory (must contain app.json)',
    );
    if (dir == null) return;

    setState(() => _installStatus = 'Installing...');
    try {
      // Derive a self-signed credential from the directory name
      final appName = dir.split(RegExp(r'[/\\]')).last;
      final credential = await generateAppCredential(appName: appName);
      final res = await verifyAndInstallApp(zipPath: dir, credentialSig: credential);
      if (mounted) {
        setState(() => _installStatus = res);
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Installed: $appName')));
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          const Text('App Vault', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold, fontFamily: 'monospace')),
                        ],
                      ),
                      ElevatedButton.icon(
